...
function StartScreen({ onStart, questions, certInfo, certPath }: { onStart: () => void; questions: ExamQuestion[]; certInfo: CertInfo; certPath: string }) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-8 text-center">
        <div className="h-16 w-16 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
          <svg className="w-8 h-8 text-sap-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        </div>
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Simulateur d'examen — {certInfo.code}</h2>
        <p className="text-slate-600 dark:text-slate-400 mb-6">{certInfo.shortName}</p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            ...
            <div key={item.label} className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3">
              <p className="text-xl font-black text-sap-blue">{item.value}</p>
        ...
        <button onClick={onStart} className="w-full py-3 bg-sap-blue text-white rounded-xl font-bold text-base hover:bg-sap-blue-dark transition-colors">Démarrer l'examen</button>
        <Link href={certPath} className="block mt-3 text-sm text-slate-500 hover:text-sap-blue transition-colors">← Retour aux chapitres</Link>
      </div>
    </motion.div>
  );
}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setReviewMode(false)} className="text-sm text-sap-blue hover:underline">Retour aux résultats</button>
          </div>
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button onClick={() => setReviewIndex((i) => Math.min(questions.length - 1, i + 1))} disabled={reviewIndex === questions.length - 1} className="px-4 py-2 bg-sap-blue text-white rounded-xl text-sm font-semibold disabled:opacity-40">Suivante →</button>
          </div>
        </div>
      </motion.div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button onClick={() => setReviewMode(true)} className="flex-1 py-2.5 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-sm font-semibold hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">Revoir les questions</button>
        <Link href={certPath} className="flex-1 py-2.5 bg-sap-blue text-white rounded-xl text-sm font-semibold text-center hover:bg-sap-blue-dark transition-colors">Retour aux chapitres</Link>
      </div>
      ...
      {!passed && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 text-sm text-blue-800 dark:text-blue-200">
          <p className="font-semibold mb-1">Conseils pour progresser :</p>
          <ul className="space-y-1">
            {Object.entries(byChapter)
              .filter(([, data]) => data.correct / data.total < 0.65)
              .map(([chapter]) => <li key={chapter}>• Révisez le chapitre : <strong>{chapter}</strong></li>)
            }
          </ul>
        </div>
      )}
...
interface ExamSimulatorTemplateProps {
  questions: ExamQuestion[];
  certInfo: CertInfo;
  moduleId: string;
  certPath: string;
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-gray-50 dark:bg-sap-dark">
        ...
        <div className="sticky top-16 z-30 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-4 py-2">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Q {currentQ + 1}/{questions.length}</span>
              <span className="text-xs text-slate-400">{answeredCount}/{questions.length} répondues</span>
            </div>
            ...
            <button onClick={handleSubmit} className="px-4 py-1.5 bg-sap-blue text-white rounded-xl text-sm font-semibold hover:bg-sap-blue-dark transition-colors">Terminer l'examen</button>
          </div>
          <div className="max-w-4xl mx-auto mt-2">
            <div className="h-1 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
              <div className="h-full bg-sap-blue rounded-full transition-all" style={{ width: `${(answeredCount / questions.length) * 100}%` }} />
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        {phase === "start" && <StartScreen onStart={handleStart} questions={questions} certInfo={certInfo} certPath={certPath} />}
        ...
        {phase === "exam" && (
          <div className="grid lg:grid-cols-[1fr_auto] gap-5">
            <motion.div key={currentQ} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.15 }}
              ...
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-semibold text-sap-blue dark:text-sap-accent bg-sap-blue/10 dark:bg-sap-blue/20 px-2.5 py-1 rounded-lg">{q.chapter}</span>
                <button onClick={toggleFlag} title={flagged.has(currentQ) ? "Retirer le marquage" : "Marquer pour révision"}
                  className={`text-xs px-2.5 py-1 rounded-lg border transition-colors font-semibold ${flagged.has(currentQ) ? "bg-amber-100 dark:bg-amber-900/30 text-amber-600 border-amber-300 dark:border-amber-700" : "bg-gray-100 dark:bg-slate-700 text-slate-500 border-gray-200 dark:border-slate-600"}`}>
              ...
              <div className="space-y-2">
                {q.options.map((opt, idx) => (
                  <button key={idx} onClick={() => handleAnswer(idx)}
                    className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${answers[currentQ] === idx ? "border-sap-blue bg-sap-blue/10 dark:bg-sap-blue/20 text-sap-blue dark:text-sap-accent font-semibold" : "border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-sap-blue/40 hover:bg-gray-50 dark:hover:bg-slate-700"}`}>
                    <span className="inline-flex items-center gap-2">
                      <span className={`h-6 w-6 rounded-full border-2 flex items-center justify-center text-xs font-bold flex-shrink-0 ${answers[currentQ] === idx ? "border-sap-blue bg-sap-blue text-white" : "border-gray-300 dark:border-slate-600"}`}>
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100 dark:border-slate-700">
                <button onClick={() => setCurrentQ((c) => Math.max(0, c - 1))} disabled={currentQ === 0} className="px-4 py-2 bg-gray-100 dark:bg-slate-700 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-600 disabled:opacity-40 transition-colors">← Précédente</button>
                <span className="text-xs text-slate-400">{answers[currentQ] !== null ? "✓ Répondue" : "Non répondue"}</span>
                <button onClick={() => setCurrentQ((c) => Math.min(questions.length - 1, c + 1))} disabled={currentQ === questions.length - 1} className="px-4 py-2 bg-sap-blue text-white rounded-xl text-sm font-semibold hover:bg-sap-blue-dark disabled:opacity-40 transition-colors">Suivante →</button>
              </div>
            </motion.div>
                ...
                <div className="grid grid-cols-5 gap-1">
                  {questions.map((_, i) => (
                    <button key={i} onClick={() => setCurrentQ(i)}
                      className={`h-8 w-8 rounded-lg text-xs font-bold transition-colors ${i === currentQ ? "bg-sap-blue text-white" : flagged.has(i) ? "bg-amber-100 dark:bg-amber-900/30 text-amber-600 border border-amber-300 dark:border-amber-700" : answers[i] !== null ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400" : "bg-gray-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:bg-gray-200 dark:hover:bg-slate-600"}`}>
                      {i + 1}
                    </button>
                  ))}
                </div>
                <div className="mt-3 space-y-1.5">
                  <div className="flex items-center gap-2 text-xs text-slate-500"><div className="h-3 w-3 rounded bg-emerald-100 dark:bg-emerald-900/30 border border-emerald-200" />Répondue</div>
                  <div className="flex items-center gap-2 text-xs text-slate-500"><div className="h-3 w-3 rounded bg-amber-100 dark:bg-amber-900/30 border border-amber-300" />Marquée</div>
                  <div className="flex items-center gap-2 text-xs text-slate-500"><div className="h-3 w-3 rounded bg-gray-100 dark:bg-slate-700" />Non répondue</div>
                </div>
              </div>
            </div>
          </div>
