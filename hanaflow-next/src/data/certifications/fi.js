/**
 * Contenu de la certification C_TS4FI_2601
...
export const fiCertification = {
  ...
  simulatorQuestions: 80,  // questions disponibles dans notre simulateur (= format examen réel)
  ...
  chapters: [
    ...
    // ═══════════════════════════════════════════════════════════════
    {
      ...
      lessons: [
        ...
        {
          id: "l6-2",
          title: "Programme de paiement automatique (F110), méthodes de paiement et gestion de la trésorerie avancée",
          content: [
            "Le Programme de Paiement Automatique (Automatic Payment Program, transaction F110) est l'outil central pour générer en masse les paiements fournisseurs et les encaissements clients. Il sélectionne les pièces ouvertes (factures non payées) selon des critères, propose une liste de paiement, et après validation génère les écritures comptables et les fichiers bancaires (virements, prélèvements). C'est la transaction la plus utilisée par les comptables fournisseurs en routine quotidienne.",
            "Le déroulement F110 suit 5 étapes : 1) Paramétrage (Parameters) — date d'exécution, sociétés, méthodes de paiement, plage fournisseurs ; 2) Proposition (Proposal Run) — SAP simule le paiement et génère une liste à valider ; 3) Validation/Modification — possibilité d'exclure des fournisseurs, modifier des montants ; 4) Exécution réelle (Payment Run) — génération des écritures Débit Fournisseur / Crédit Banque ; 5) Génération des fichiers de paiement (DME — Data Medium Exchange) à transmettre à la banque.",
            "Les Méthodes de Paiement (Payment Methods) sont configurées dans FBZP. Méthodes courantes : C (chèque), T (virement bancaire SCT/SEPA Credit Transfer), D (prélèvement SDD/SEPA Direct Debit), W (virement international SWIFT). Chaque méthode est associée à un format de fichier bancaire (CGI XML pour SEPA, IDOC pour traitements internes) et à une stratégie d'optimisation (regrouper plusieurs factures en un seul paiement).",
            "La Sélection des Pièces (Document Selection) en F110 utilise plusieurs critères : date d'échéance ≤ date de prochaine exécution, méthode de paiement de la fiche fournisseur, blocage paiement (Payment Block), validation préalable. Les escomptes (cash discounts) sont calculés automatiquement si la facture est dans la période d'escompte. Le Payment Run génère systématiquement l'écriture d'escompte si applicable.",
            "Le Cash Concentration (centralisation de trésorerie) permet à un groupe de regrouper automatiquement les soldes des filiales sur un compte central. Le Cash Pooling SAP-TR gère les transferts intercompagnies et le calcul des intérêts internes. Pour les multinationales, ces fonctions sont essentielles pour optimiser la trésorerie globale et réduire les coûts financiers.",
            "Le Liquidity Forecast (prévision de liquidité, FF7B) projette la trésorerie sur plusieurs semaines/mois en intégrant les pièces ouvertes (créances clients, dettes fournisseurs avec leurs dates d'échéance), les engagements d'achats (BC ouverts), les commandes clients (encaissements futurs), les budgets. Outil clé pour anticiper les besoins de financement court terme."
          ],
          keyConcepts: [
            { term: "F110 — Payment Run", definition: "Programme central de paiement automatique. 5 étapes : Parameters, Proposal, Modification, Run, DME files. Génère écritures + fichiers bancaires." },
            { term: "FBZP", definition: "Configuration des méthodes de paiement, formats DME, banques utilisées par méthode. Paramétrage complexe à maîtriser pour la cert." },
            { term: "DME (Data Medium Exchange)", definition: "Fichier bancaire généré par F110 (formats CGI XML pour SEPA, MT103 SWIFT). Transmis à la banque pour exécuter les paiements." },
            { term: "SEPA SCT / SDD", definition: "Standards européens : SCT (Credit Transfer = virement), SDD (Direct Debit = prélèvement). Format XML CGI obligatoire dans la zone euro." },
            { term: "Payment Block", definition: "Indicateur qui empêche temporairement le paiement d'une facture (litige, vérif en cours). F110 exclut automatiquement ces pièces." },
            { term: "Liquidity Forecast (FF7B)", definition: "Prévision de trésorerie à moyen terme intégrant pièces ouvertes, BC, commandes clients. Outil de pilotage trésorier." },
            { term: "Cash Concentration", definition: "Centralisation automatique des soldes des filiales vers un compte groupe. SAP TR-CM gère le cash pooling intercompagnies." },
          ],
          tcodes: [
            { code: "F110", description: "Programme de paiement automatique fournisseurs/clients" },
            { code: "FBZP", description: "Configurer les méthodes de paiement et formats DME" },
            { code: "FB05", description: "Lettrage manuel de pièces (poste ouvert avec contrepartie)" },
            { code: "F-44", description: "Compensation manuelle d'un compte fournisseur" },
            { code: "FF7B", description: "Liquidity Forecast (prévision de trésorerie)" },
            { code: "F-58", description: "Paiement manuel (chèque, virement individuel)" },
          ],
        },
      ],
      quiz: [
...
export const fiMockExamQuestions = [
  ...
  // Ch1 — Vue d'ensemble (+3 questions)
  { id: "exam-41", chapter: "Vue d'ensemble SAP S/4HANA Finance", question: "Quelle technologie permet le traitement temps réel dans SAP S/4HANA ?", options: ["MaxDB", "Oracle DB", "SAP HANA (base In-Memory)", "Microsoft SQL Server"], correctIndex: 2, explanation: "SAP HANA est une base de données In-Memory : les données sont stockées dans la RAM, ce qui permet un traitement analytique et transactionnel en temps réel sans batch jobs nocturnes." },
  { id: "exam-42", chapter: "Vue d'ensemble SAP S/4HANA Finance", question: "Le périmètre de contrôle de gestion (Controlling Area) permet :", options: ["De gérer la comptabilité légale d'une société", "De regrouper plusieurs sociétés pour le reporting CO et l'imputation des coûts", "De définir le plan comptable", "De configurer les exercices fiscaux"], correctIndex: 1, explanation: "Le Controlling Area regroupe une ou plusieurs sociétés pour permettre les imputations CO et le reporting analytique transversal. Une société doit lui être assignée pour que les coûts soient imputés." },
  { id: "exam-43", chapter: "Vue d'ensemble SAP S/4HANA Finance", question: "Combien de périodes spéciales SAP propose-t-il en plus des 12 périodes mensuelles standards ?", options: ["1", "2", "4", "12"], correctIndex: 2, explanation: "SAP propose 4 périodes spéciales (13, 14, 15, 16) en plus des 12 périodes mensuelles. Elles servent aux écritures de clôture annuelle (provisions, ajustements, audits) sans impacter le mois en cours." },
  // Ch2 — Comptabilité Générale (+10 questions)
  { id: "exam-44", chapter: "Comptabilité Générale", question: "La transaction OB52 dans SAP FI permet de :", options: ["Définir le plan comptable", "Gérer l'ouverture/fermeture des périodes comptables", "Créer un compte général", "Configurer les variantes d'exercice"], correctIndex: 1, explanation: "OB52 (Posting Periods) configure les périodes comptables ouvertes pour la saisie. SAP empêche les saisies dans les périodes fermées pour préserver l'intégrité de la comptabilité." },
  { id: "exam-45", chapter: "Comptabilité Générale", question: "Le plan comptable opérationnel (Operating Chart of Accounts) en SAP est :", options: ["Le plan utilisé pour les ventes uniquement", "Le plan principal utilisé pour les saisies courantes (FI et CO)", "Un plan obsolète remplacé en S/4HANA", "Le plan utilisé pour la consolidation"], correctIndex: 1, explanation: "Le plan opérationnel (ex: INT, CAFR) est le plan principal de l'entreprise pour FI et CO. SAP supporte aussi un plan groupe (consolidation) et un plan pays (légal) en parallèle." },
  { id: "exam-46", chapter: "Comptabilité Générale", question: "La transaction FB50 (saisie d'écriture FI Express) est :", options: ["Une saisie pour la trésorerie uniquement", "Une saisie simplifiée d'écriture GL avec validation rapide (Enjoy SAP)", "Une transaction obsolète remplacée par FB01", "Un programme de saisie en masse"], correctIndex: 1, explanation: "FB50 est une transaction Enjoy SAP qui simplifie la saisie d'écritures GL (interface tabulaire moderne). FB01 reste la saisie classique multi-postes plus complète." },
  { id: "exam-47", chapter: "Comptabilité Générale", question: "Dans SAP S/4HANA, la table BSEG existe encore pour :", options: ["Stocker les nouvelles écritures comptables", "Compatibilité ascendante (vues de l'Universal Journal ACDOCA)", "Le module CO uniquement", "Les écritures de groupe"], correctIndex: 1, explanation: "Dans S/4HANA, BSEG et BKPF existent comme vues techniques au-dessus de l'Universal Journal ACDOCA, pour assurer la compatibilité avec les programmes ABAP legacy. ACDOCA reste la source unique de vérité." },
  { id: "exam-48", chapter: "Comptabilité Générale", question: "Le concept de Document Splitting (Ventilation des pièces) en SAP S/4HANA permet :", options: ["De fractionner un compte en sous-comptes", "De ventiler automatiquement une écriture sur plusieurs dimensions (segment, profit center, ...) pour produire un bilan par segment", "De séparer les pièces en plusieurs périodes", "De diviser une facture en plusieurs paiements"], correctIndex: 1, explanation: "Le Document Splitting ventile chaque écriture sur les dimensions de reporting (segment, profit center, business area) pour permettre des bilans/comptes de résultat par segment. Activé via la configuration nouvelle compta GL." },
  { id: "exam-49", chapter: "Comptabilité Générale", question: "Une pièce comptable récurrente (Recurring Document) en SAP est :", options: ["Une écriture qui se répète mensuellement automatiquement", "Un modèle d'écriture stocké, à exécuter manuellement aux dates voulues via F.14", "Une écriture avec montant variable", "Une écriture qui ne peut pas être annulée"], correctIndex: 1, explanation: "FBD1 crée le modèle de pièce récurrente. F.14 exécute périodiquement les pièces récurrentes échues à la date demandée (loyers, abonnements, dotations régulières)." },
  { id: "exam-50", chapter: "Comptabilité Générale", question: "Une note de débit/crédit dans SAP est saisie via :", options: ["FB01 ou FB60 (fournisseur), FB70 (client) avec type de pièce approprié", "Une transaction MM uniquement", "Un programme de paiement", "Le programme batch automatique"], correctIndex: 0, explanation: "Les notes de débit/crédit sont des écritures FI standard avec un type de pièce qui les identifie (KG = avoir fournisseur, DG = avoir client). FB01/FB60/FB70 selon le module concerné (GL/AP/AR)." },
  { id: "exam-51", chapter: "Comptabilité Générale", question: "Le numéro d'une pièce comptable SAP est :", options: ["Toujours saisi par l'utilisateur", "Géré par une plage de numérotation (Number Range) configurée par type de pièce et exercice", "Identique au numéro de facture fournisseur", "Aléatoire"], correctIndex: 1, explanation: "Chaque type de pièce (DR, KR, SA, etc.) est associé à une plage de numérotation (Number Range) configurée par exercice. SAP attribue automatiquement le numéro suivant lors de la comptabilisation." },
  { id: "exam-52", chapter: "Comptabilité Générale", question: "L'extension de compte GL (Field Status Group) détermine :", options: ["La couleur des comptes dans l'écran de saisie", "Quels champs sont obligatoires/optionnels/cachés lors de la saisie d'une écriture sur ce compte", "Le solde maximum autorisé", "Le type de monnaie"], correctIndex: 1, explanation: "Le Field Status Group (groupe de statut de champs) contrôle les champs lors de la saisie : obligatoire, optionnel, supprimé. Configuré via OBC4 et associé aux comptes GL pour piloter la qualité des saisies." },
  { id: "exam-53", chapter: "Comptabilité Générale", question: "L'inversion d'une pièce comptable dans SAP utilise :", options: ["FB01 avec montants négatifs", "FB08 (Reverse Document) qui crée automatiquement la contre-écriture", "Une suppression directe de la pièce", "Une modification de la pièce existante"], correctIndex: 1, explanation: "FB08 (Reverse Document) crée une contre-écriture qui annule la pièce originale. La pièce d'origine n'est jamais supprimée — l'audit trail est préservé. F.80 fait l'inversion en masse." },
  // Ch3 — Comptabilité Fournisseurs (+6 questions)
  { id: "exam-54", chapter: "Comptabilité Fournisseurs", question: "Dans S/4HANA, la fiche fournisseur est gérée via :", options: ["FK01 / XK01 uniquement", "L'objet Business Partner (transaction BP) avec rôle FLVN00/FLVN01", "MM01", "FD01"], correctIndex: 1, explanation: "S/4HANA unifie clients et fournisseurs dans le Business Partner (BP). Les rôles FLVN00 (général) et FLVN01 (fournisseur) qualifient un BP comme fournisseur. FK01/XK01 restent fonctionnelles mais déléguent à BP." },
  { id: "exam-55", chapter: "Comptabilité Fournisseurs", question: "Le compte de réconciliation fournisseur (Reconciliation Account) :", options: ["Est saisi manuellement à chaque écriture", "Est défini dans la fiche fournisseur et reçoit automatiquement les écritures GL parallèles aux écritures auxiliaires", "Est un compte de transition", "Est un compte client"], correctIndex: 1, explanation: "Le compte de réconciliation est le compte GL qui reflète automatiquement les écritures du livre auxiliaire (fournisseur). Il assure que la balance auxiliaire = balance générale en permanence." },
  { id: "exam-56", chapter: "Comptabilité Fournisseurs", question: "FB60 dans SAP FI sert à :", options: ["Créer un fournisseur", "Saisir une facture fournisseur sans BC (frais directs, frais bancaires)", "Comptabiliser un paiement", "Réviser le compte fournisseur"], correctIndex: 1, explanation: "FB60 (Enter Vendor Invoice) saisit une facture fournisseur directe (sans BC associé). Pour les factures avec BC, on utilise MIRO côté MM. FB60 = Express Invoice Entry." },
  { id: "exam-57", chapter: "Comptabilité Fournisseurs", question: "Une retenue à la source (Withholding Tax) en SAP FI est :", options: ["Un escompte commercial", "Un montant prélevé sur le paiement et reversé directement à l'administration fiscale (ex: BNC en France)", "Une majoration TVA", "Un blocage de paiement temporaire"], correctIndex: 1, explanation: "La Withholding Tax (TDS / Retenue à la Source) est un montant prélevé sur le paiement fournisseur et reversé à l'État. Configurée via fiche fournisseur + types de WT. Importante en France pour les BNC, à l'international pour les non-résidents." },
  { id: "exam-58", chapter: "Comptabilité Fournisseurs", question: "L'âge des dettes fournisseurs (Aging Report) est consulté avec :", options: ["FBL1N avec sélection par échéance, ou rapport S_ALR_87012077", "MIGO", "FF7A", "F-44"], correctIndex: 0, explanation: "FBL1N affiche les pièces ouvertes fournisseur avec dates d'échéance. Le rapport S_ALR_87012077 (Vendor Aging) classe les dettes par tranche d'ancienneté (à échoir, échues 0-30j, 30-60j, etc.)." },
  { id: "exam-59", chapter: "Comptabilité Fournisseurs", question: "Un acompte fournisseur (Down Payment) est comptabilisé via :", options: ["F-47 (demande d'acompte) puis F-48 (paiement de l'acompte)", "FB60 avec montant négatif", "MIRO directement", "F110 uniquement"], correctIndex: 0, explanation: "Processus acompte : F-47 crée la demande d'acompte (compte spécial), F-48 paye l'acompte au fournisseur. À la facture finale, F-54 lettre l'acompte avec la facture pour ne payer que le solde." },
  { id: "exam-60", chapter: "Comptabilité Fournisseurs", question: "Le blocage de paiement (Payment Block) sur une facture fournisseur :", options: ["Annule la facture", "Empêche temporairement le paiement par F110 jusqu'à validation manuelle", "Bloque la création de nouveaux BC vers ce fournisseur", "Supprime la dette fournisseur"], correctIndex: 1, explanation: "Le Payment Block (champ ZLSPR) est posé manuellement (litige, vérif) ou automatiquement (par MIRO sur écart prix). F110 exclut les pièces bloquées. MRBR ou modification manuelle pour débloquer." },
  // Ch4 — Comptabilité Clients (+6 questions)
  { id: "exam-61", chapter: "Comptabilité Clients", question: "La création d'un client dans S/4HANA passe par :", options: ["FD01", "BP avec rôle FLCU00/FLCU01 (Customer)", "VD01 uniquement", "OBA1"], correctIndex: 1, explanation: "S/4HANA : le client est un Business Partner avec rôles FLCU00 (général) + FLCU01 (Customer). Les rôles SD complémentaires (FLCU01) ajoutent les vues ventes. BP est l'unique transaction de gestion." },
  { id: "exam-62", chapter: "Comptabilité Clients", question: "Le programme de relance (Dunning Program, F150) :", options: ["Génère des paiements clients", "Identifie les pièces clients en retard de paiement et génère des courriers/e-mails de relance par niveau", "Calcule la TVA", "Compense les comptes clients"], correctIndex: 1, explanation: "F150 analyse les pièces ouvertes clients selon les niveaux de relance configurés (1=rappel doux, 2=relance ferme, 3=mise en demeure). Génère lettres et historise les niveaux dans le BP." },
  { id: "exam-63", chapter: "Comptabilité Clients", question: "FB70 sert à :", options: ["Saisir une facture client (sans SD)", "Créer un client", "Gérer le crédit client", "Comptabiliser un encaissement"], correctIndex: 0, explanation: "FB70 (Customer Invoice Entry) saisit une facture client directement en FI sans flux SD (frais divers, refacturation, intérêts). Les ventes commerciales passent par SD (VF01)." },
  { id: "exam-64", chapter: "Comptabilité Clients", question: "L'encours client (Customer Credit Limit) est géré dans SAP via :", options: ["Le module SD (Credit Management) avec contrôle automatique lors des commandes clients", "FBL5N uniquement", "MM03", "OBYC"], correctIndex: 0, explanation: "Le Credit Management (FSCM-CR) définit une limite de crédit par client. Lors des commandes (VA01) ou livraisons, SAP vérifie le dépassement de limite (encours + nouvelle commande > limite) et bloque ou avertit." },
  { id: "exam-65", chapter: "Comptabilité Clients", question: "F-28 dans SAP FI sert à :", options: ["Créer un client", "Saisir un encaissement client (paiement entrant) avec lettrage des factures payées", "Annuler une facture client", "Configurer le programme de relance"], correctIndex: 1, explanation: "F-28 (Incoming Payment) saisit un encaissement client : montant reçu sur un compte bancaire, sélection des pièces ouvertes du client à lettrer. Génère écriture Débit Banque / Crédit Client." },
  { id: "exam-66", chapter: "Comptabilité Clients", question: "Un avoir client (Customer Credit Memo) est :", options: ["Une facture avec montant négatif", "Une pièce avec type de pièce DG (généralement) qui réduit la dette du client", "Un blocage du compte client", "Un acompte"], correctIndex: 1, explanation: "Un avoir client utilise généralement le type de pièce DG (Customer Credit Memo). Il diminue le solde dû par le client (ex: retour marchandise, geste commercial). Comptabilisé via FB75 ou flux SD." },
  // Ch5 — Comptabilité Immobilisations (+5 questions)
  { id: "exam-67", chapter: "Comptabilité des Immobilisations", question: "Les classes d'immobilisations (Asset Classes) en SAP regroupent :", options: ["Les fournisseurs par catégorie", "Les types d'immobilisations partageant les mêmes règles d'amortissement et comptes (ex: bâtiments, véhicules)", "Les centres de coûts", "Les périodes d'amortissement"], correctIndex: 1, explanation: "Une Asset Class (classe d'immobilisations) définit les règles communes : comptes GL d'imputation, durée et méthode d'amortissement, schéma d'amortissement. Ex: Class 1100 = Bâtiments, 2100 = Véhicules. Configurée via OAOA." },
  { id: "exam-68", chapter: "Comptabilité des Immobilisations", question: "AS01 dans SAP FI-AA sert à :", options: ["Créer une immobilisation (asset master)", "Calculer l'amortissement", "Vendre une immobilisation", "Effectuer l'inventaire des immobilisations"], correctIndex: 0, explanation: "AS01 crée la fiche immobilisation (Asset Master Record) avec ses données : nom, classe, centre coûts, durée d'amortissement, dates. AS02 modifie, AS03 affiche." },
  { id: "exam-69", chapter: "Comptabilité des Immobilisations", question: "AFAB est le programme :", options: ["De cession d'immobilisation", "D'exécution de l'amortissement périodique (Depreciation Run)", "De création d'immobilisation", "De réévaluation"], correctIndex: 1, explanation: "AFAB (Periodic Posting Run) exécute l'amortissement périodique : calcule la dépréciation du mois et génère les écritures comptables (Débit Dotations amortissements / Crédit Amortissements cumulés). Lancé en clôture mensuelle." },
  { id: "exam-70", chapter: "Comptabilité des Immobilisations", question: "Une cession d'immobilisation par vente est saisie via :", options: ["F-92 (Asset Sale with Customer)", "AFAB", "AS01", "FB60"], correctIndex: 0, explanation: "F-92 enregistre la cession avec produit de cession (vente à un client). SAP calcule automatiquement la valeur nette comptable, génère la sortie d'actif et le résultat de cession (plus/moins-value)." },
  { id: "exam-71", chapter: "Comptabilité des Immobilisations", question: "L'aire d'évaluation (Depreciation Area) en SAP FI-AA permet :", options: ["De stocker plusieurs valorisations parallèles d'une même immobilisation (ex: comptabilité locale 01, IFRS 30, fiscale 15)", "De définir le centre de coûts", "De gérer les sous-numéros", "De configurer la classe"], correctIndex: 0, explanation: "Les Depreciation Areas (ex: 01 = comptable locale, 30 = IFRS, 15 = fiscale) permettent de tenir plusieurs jeux de valorisation pour la même immobilisation. SAP comptabilise séparément chaque aire pour le reporting multi-référentiel." },
  // Ch6 — Gestion de Trésorerie (+3 questions)
  { id: "exam-72", chapter: "Gestion de Trésorerie", question: "F110 dans SAP FI est :", options: ["Le programme de paiement automatique fournisseurs/clients", "Une transaction de saisie de relevé bancaire", "Un programme de relance client", "Une transaction d'amortissement"], correctIndex: 0, explanation: "F110 (Automatic Payment Run) génère en masse les paiements fournisseurs (et encaissements clients par SDD). 5 étapes : Paramètres, Proposition, Modification, Exécution, fichiers DME pour la banque." },
  { id: "exam-73", chapter: "Gestion de Trésorerie", question: "Le format SEPA SCT (Credit Transfer) est :", options: ["Un format de prélèvement", "Le format XML standard européen pour les virements en zone euro", "Un format propriétaire SAP", "Un format obsolète"], correctIndex: 1, explanation: "SEPA SCT (Single Euro Payments Area Credit Transfer) est le format XML CGI ISO 20022 obligatoire pour les virements en zone euro depuis 2014. SAP F110 génère ce format via la configuration FBZP." },
  { id: "exam-74", chapter: "Gestion de Trésorerie", question: "FBZP dans SAP FI configure :", options: ["Les méthodes de paiement, formats DME et banques par méthode", "Les fournisseurs", "Le grand livre", "Les comptes bancaires uniquement"], correctIndex: 0, explanation: "FBZP est le paramétrage central du programme de paiement : sociétés, méthodes (T=virement, D=prélèvement), formats DME (CGI XML SEPA, MT103), banques par méthode, classement de paiement. Complexe mais essentiel." },
  // Ch7 — Opérations de Clôture (+7 questions)
  { id: "exam-75", chapter: "Opérations de Clôture", question: "La transaction F.05 (Foreign Currency Valuation) :", options: ["Convertit les pièces en devise de société à la date du jour", "Réévalue les postes ouverts en devise étrangère au taux de clôture et comptabilise les écarts (gains/pertes de change non réalisés)", "Bloque les pièces en devise étrangère", "Crée des pièces en devise"], correctIndex: 1, explanation: "F.05 réévalue les soldes en devise étrangère (créances, dettes, banques) au taux de clôture. Génère écritures Débit/Crédit Gains/Pertes de change non réalisés. Étape obligatoire en clôture mensuelle/annuelle." },
  { id: "exam-76", chapter: "Opérations de Clôture", question: "Le 'Closing Cockpit' (Cockpit de clôture) en SAP est :", options: ["Une cabine de pilotage avion", "L'outil pilote orchestré qui exécute et trace toutes les étapes de la clôture mensuelle (FI, CO, MM, ...) avec dépendances", "Une interface uniquement consultative", "Un programme de reporting"], correctIndex: 1, explanation: "Le Closing Cockpit (transaction CLOCO en S/4HANA, ex SCMA en ECC) orchestre les programmes de clôture multi-modules avec gestion des dépendances, statuts, owners. Permet une clôture rapide, traçable et auditée." },
  { id: "exam-77", chapter: "Opérations de Clôture", question: "L'arrêté des stocks à la clôture (transaction MMRV/MMPV pour MM, F.02 pour FI) sert à :", options: ["Bloquer les saisies", "Vérifier que les périodes MM et FI sont synchronisées avant la clôture", "Calculer la valeur totale des stocks", "Lancer le MRP"], correctIndex: 1, explanation: "MMRV/MMPV gèrent les périodes MM (basculement). F.02 (legacy) ou ses équivalents en S/4HANA vérifient la cohérence FI-MM. La désynchronisation des périodes est une erreur fréquente en clôture." },
  { id: "exam-78", chapter: "Opérations de Clôture", question: "Le report des soldes à nouveau (Year-End Carry Forward) en SAP utilise :", options: ["F.16 / FAGLGVTR pour les comptes de bilan, automatique en S/4HANA", "FB01", "F-02", "OBYC"], correctIndex: 0, explanation: "F.16 (legacy) ou FAGLGVTR (nouvelle compta GL) reportent les soldes des comptes de bilan dans la nouvelle année. Les comptes de résultat (charges/produits) sont automatiquement soldés vers le résultat de l'exercice." },
  { id: "exam-79", chapter: "Opérations de Clôture", question: "Le journal de clôture (Closing Journal) en SAP contient :", options: ["Toutes les pièces de l'année", "Les écritures spécifiques de clôture : provisions, ajustements, OD, etc., souvent en périodes spéciales 13-16", "Uniquement les écritures GL", "Les écritures consolidées uniquement"], correctIndex: 1, explanation: "Les écritures de clôture (provisions de fin d'année, ajustements, OD audit) sont saisies dans les périodes spéciales 13-16 pour ne pas perturber les chiffres mensuels. Pratique courante pour préserver la lisibilité." },
  { id: "exam-80", chapter: "Opérations de Clôture", question: "Le rapport Bilan + Compte de Résultat S/4HANA recommandé est :", options: ["F.01 (legacy)", "S_ALR_87012284 (legacy)", "Les apps Fiori 'Display Financial Statement' / 'Trial Balance' basées sur ACDOCA en temps réel", "FBL3N"], correctIndex: 2, explanation: "En S/4HANA, les apps Fiori 'Display Financial Statement', 'Trial Balance', 'P&L Statement' s'appuient directement sur ACDOCA pour un reporting temps réel. F.01 et S_ALR_* restent disponibles mais Fiori est l'expérience moderne recommandée." },
];
