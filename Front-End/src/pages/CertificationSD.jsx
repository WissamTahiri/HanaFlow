...
function QuizBlock({ quiz }) {
  ...
  const handleConfirm = () => {
    ...
    setShowResult(true);
    setAnswers((prev) => [...prev, { questionId: q.id, correct: selected === q.correctIndex }]);
  };
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
          Score : {pct}% {passed ? "— Seuil atteint (65%)" : "— Seuil : 65%"}
        </p>
        <button
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
        ...
        <div className="flex gap-1">
          {quiz.map((_, i) => (
            <div key={i} className={`h-1.5 w-6 rounded-full transition-colors ${i < current ? "bg-emerald-400" : i === current ? "bg-purple-600" : "bg-gray-200 dark:bg-slate-700"}`} />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {lesson.keyConcepts?.length > 0 && (
        <div>
          <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
            <span className="h-1.5 w-6 bg-purple-600 rounded-full" />
            Concepts clés
          </h4>
          <div className="grid gap-2">
            {lesson.keyConcepts.map((c, i) => (
              <div key={i} className="bg-purple-50 dark:bg-purple-900/20 rounded-xl px-4 py-3 border border-purple-100 dark:border-purple-800">
                <span className="font-semibold text-sm text-purple-900 dark:text-purple-200">{c.term}</span>
      ...
      {lesson.tcodes?.length > 0 && (
        <div>
          <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
            <span className="h-1.5 w-6 bg-purple-500 rounded-full" />
                ...
                {lesson.tcodes.map((t, i) => (
                  <tr key={i} className="border-t border-gray-100 dark:border-slate-700">
                    <td className="px-4 py-2.5">
                      <code className="bg-purple-600/10 dark:bg-purple-600/20 text-purple-700 dark:text-purple-400 px-2 py-0.5 rounded font-mono text-xs font-bold">
                        {t.code}
                      </code>
                    </td>
...
export default function CertificationSD() {
  ...
  const [completedLessons, setCompletedLessons] = useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem("sd_completed_lessons") || "[]")); }
    catch { return new Set(); }
  });
  const [showUpgradeModal, setShowUpgradeModal] = useState(false);

  const cert = sdCertification;
  ...
  return (
    <>
      <SEO
        title="Certification SAP SD — C_TS4SD_2023"
        description="Préparez la certification SAP Sales and Distribution (C_TS4SD_2023) avec 7 chapitres, des quiz et un simulateur d'examen de 40 questions."
            ...
            >
              <div className="text-center mb-5">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">
                  Ce chapitre est réservé aux membres Pro. Passez au plan Pro pour accéder à tous les chapitres SD et au simulateur d'examen.
                </p>
              </div>
              <div className="space-y-2 mb-5">
                {["7 chapitres SD complets (C_TS4SD_2023)", "Simulateur d'examen 40 questions", "Quiz détaillés avec explications", "Suivi de progression complet"].map((f) => (
      ...
      <div className="min-h-[calc(100vh-4rem)]">
        <div className="bg-gradient-to-br from-purple-900 via-purple-700 to-purple-500 text-white">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
            <nav className="flex items-center gap-2 text-xs text-white/60 mb-4">
              ...
              <span>/</span>
              <span className="text-white/90">SAP SD</span>
            </nav>
              ...
              <aside className="hidden lg:block w-72 flex-shrink-0 sticky top-20">
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-xs text-slate-400 dark:text-slate-500">{chLessonsCompleted}/{ch.lessons.length} leçons</span>
                  ...
                  <div className="px-4 py-3 border-t border-gray-100 dark:border-slate-700">
                    {canAccess(true) ? (
                      <Link to="/certifications/sd/examen"
              ...
              <main className="flex-1 min-w-0">
                    ...
                    <div className="px-6 pt-4 flex flex-wrap gap-2 border-b border-gray-100 dark:border-slate-700 pb-3">
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors
                          ${showQuiz ? "bg-orange-500 text-white" : "bg-gray-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-600"}`}
                      >
                ...
                <div className="lg:hidden mt-4 bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                  ...
                  <div className="px-4 py-3">
                    {canAccess(true) ? (
                      <Link to="/certifications/sd/examen" className="block w-full text-center py-2.5 bg-purple-600 text-white rounded-xl text-sm font-semibold">
                        🎯 Simuler l'examen
                      </Link>
                    ) : (
