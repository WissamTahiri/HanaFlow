    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-8 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
          Simulateur d'examen — C_THR81_2311
        </h2>
        <p className="text-slate-600 dark:text-slate-400 mb-6">SAP Human Capital Management</p>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            { label: "Questions", value: hcmMockExamQuestions.length },
            ...
            <div key={item.label} className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3">
              <p className="text-xl font-black text-purple-600">{item.value}</p>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 mb-6 text-left">
          ...
          <ul className="text-sm text-amber-700 dark:text-amber-400 space-y-1">
            <li>• {hcmMockExamQuestions.length} questions à choix multiple, 4 options chacune</li>
        ...
        <button onClick={onStart} className="w-full py-3 bg-purple-600 text-white rounded-xl font-bold text-base hover:bg-purple-700 transition-colors">
          Démarrer l'examen
        </button>
        <Link to="/certifications/hcm" className="block mt-3 text-sm text-slate-500 hover:text-purple-600 transition-colors">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-6">
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button onClick={() => setReviewIndex((i) => Math.min(questions.length - 1, i + 1))} disabled={reviewIndex === questions.length - 1}
              className="px-4 py-2 bg-purple-600 text-white rounded-xl text-sm font-semibold disabled:opacity-40">Suivante →</button>
          </div>
        </div>
      </motion.div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-5">
            ...
            return (
              <div key={chapter}>
                ...
                <div className="h-2 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${chPct >= 65 ? "bg-purple-500" : chPct >= 40 ? "bg-orange-400" : "bg-red-500"}`}
                    style={{ width: `${chPct}%` }}
                  />
                </div>
              </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Link to="/certifications/hcm"
          className="flex-1 py-2.5 bg-purple-600 text-white rounded-xl text-sm font-semibold text-center hover:bg-purple-700 transition-colors">
          Retour aux chapitres
        </Link>
      </div>
...
// ── Composant principal ───────────────────────────────────────────────────────
export default function ExamSimulatorHCM() {
  ...
  const [answers, setAnswers] = useState(new Array(hcmMockExamQuestions.length).fill(null));
  ...
  if (!canAccess(true)) {
    return (
      <>
        <SEO title="Simulateur d'examen SAP HCM" description="Accès Pro requis" path="/certifications/hcm/examen" />
        <div className="min-h-[calc(100vh-4.5rem)] bg-gray-50 dark:bg-slate-950 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-8 text-center max-w-md">
            ...
            <Link to="/certifications/hcm" className="block w-full py-2.5 bg-purple-600 text-white rounded-xl font-semibold text-sm hover:bg-purple-700 transition-colors">
              Retour — Activer Pro
            </Link>
          </div>
        </div>
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((t) => {
        if (t <= 1) {
          ...
          setAnswers((ans) => {
            const score = ans.filter((a, i) => a === hcmMockExamQuestions[i].correctIndex).length;
            onExamComplete("hcm", Math.round((score / hcmMockExamQuestions.length) * 100) >= 65);
            return ans;
          });
          setPhase("results");
          return 0;
  ...
  const handleSubmit = () => {
    clearInterval(timerRef.current);
    const score = answers.filter((a, i) => a === hcmMockExamQuestions[i].correctIndex).length;
    onExamComplete("hcm", Math.round((score / hcmMockExamQuestions.length) * 100) >= 65);
    setPhase("results");
  };
  const toggleFlag = () => {
  ...
  const q = hcmMockExamQuestions[currentQ];
  const timerColor = timeLeft < 300 ? "text-red-500" : timeLeft < 900 ? "text-orange-500" : "text-slate-700 dark:text-slate-200";

  return (
    <>
      <SEO title="Simulateur d'examen SAP HCM — C_THR81_2311" description="Simulateur 40 questions format réel SAP HCM Human Capital Management." path="/certifications/hcm/examen" />
      <div className="min-h-[calc(100vh-4.5rem)] bg-gray-50 dark:bg-slate-950">
            ...
            <div className="max-w-4xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                  Q {currentQ + 1}/{hcmMockExamQuestions.length}
                </span>
                <span className="text-xs text-slate-400">{answeredCount}/{hcmMockExamQuestions.length} répondues</span>
              </div>
              ...
              <button onClick={handleSubmit} className="px-4 py-1.5 bg-purple-600 text-white rounded-xl text-sm font-semibold hover:bg-purple-700 transition-colors">
                Terminer l'examen
              </button>
            </div>
            <div className="max-w-4xl mx-auto mt-2">
              <div className="h-1 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                <div className="h-full bg-purple-600 rounded-full transition-all" style={{ width: `${(answeredCount / hcmMockExamQuestions.length) * 100}%` }} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            <div className="grid lg:grid-cols-[1fr_auto] gap-5">
              ...
              >
                    ...
                    <button key={idx} onClick={() => handleAnswer(idx)}
                      className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${
                        answers[currentQ] === idx
                          ? "border-purple-600 bg-purple-600/10 dark:bg-purple-600/20 text-purple-700 dark:text-purple-400 font-semibold"
                          : "border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-purple-400/40 hover:bg-gray-50 dark:hover:bg-slate-700"
                      }`}>
                      <span className="inline-flex items-center gap-2">
                        <span className={`h-6 w-6 rounded-full border-2 flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                          answers[currentQ] === idx ? "border-purple-600 bg-purple-600 text-white" : "border-gray-300 dark:border-slate-600"
                        }`}>
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100 dark:border-slate-700">
                  ...
                  <button onClick={() => setCurrentQ((c) => Math.min(hcmMockExamQuestions.length - 1, c + 1))} disabled={currentQ === hcmMockExamQuestions.length - 1}
                    className="px-4 py-2 bg-purple-600 text-white rounded-xl text-sm font-semibold hover:bg-purple-700 disabled:opacity-40 transition-colors">
                    Suivante →
                  </button>
                </div>
              </motion.div>
              ...
              <div className="lg:w-48">
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-4 sticky top-32">
                    ...
                    {hcmMockExamQuestions.map((_, i) => (
                      <button key={i} onClick={() => setCurrentQ(i)}
                        className={`h-8 w-8 rounded-lg text-xs font-bold transition-colors ${
                          ...
                          : answers[i] !== null ? "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400"
                  ...
                  <div className="mt-3 space-y-1.5">
                    {[
                      { color: "bg-purple-100 dark:bg-purple-900/30 border-purple-200", label: "Répondue" },
          ...
          {phase === "results" && <ResultsScreen answers={answers} questions={hcmMockExamQuestions} timeUsed={timeUsed} />}
        </div>
      </div>
    </>
  );
