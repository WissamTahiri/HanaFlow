...
const CHECKLIST_ITEMS = [
  "Expliquer les 5 étapes du P2P dans tes mots (PR, PO, GR, Invoice, Payment).",
  "Décrire l'impact de chaque étape sur les stocks et la comptabilité FI.",
  ...
  "Identifier les principaux risques P2P (prix erronés, doublons de factures, retards GR).",
...
const MmMiniProjectChecklist = () => {
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.16 }}
      className="bg-white dark:bg-slate-800 rounded-2xl border border-orange-200 dark:border-orange-800/40 shadow-card p-6">
      <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-1">Mini-projet MM</h2>
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Prépare une mission P2P en S/4HANA. Coche chaque compétence.</p>
      <div className="mb-4">
        ...
        {CHECKLIST_ITEMS.map((item, idx) => (
          <li key={idx} className="flex items-start gap-3">
            <button type="button" onClick={() => setDone((prev) => { const c = [...prev]; c[idx] = !c[idx]; return c; })}
              className={`mt-0.5 h-5 w-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${done[idx] ? "bg-orange-400 border-orange-400 text-white" : "border-gray-300 dark:border-slate-600"}`}>
              {done[idx] && <CheckIcon />}
            </button>
