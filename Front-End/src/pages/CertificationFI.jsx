...
// ── QuizBlock ────────────────────────────────────────────────────────────────
function QuizBlock({ quiz, chapterId, onComplete }) {
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
          Score : {pct}% {passed ? "— Seuil de réussite atteint (65%)" : "— Seuil de réussite : 65%"}
        </p>
        <button
          onClick={() => { setCurrent(0); setSelected(null); setShowResult(false); setAnswers([]); setDone(false); setScore(0); }}
          className="mt-4 px-4 py-2 bg-sapBlue text-white rounded-xl text-sm font-semibold hover:bg-sapBlueDark transition-colors"
        >
          Recommencer le quiz
        </button>
      </motion.div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
        ...
        <div className="flex gap-1">
          {quiz.map((_, i) => (
            <div key={i} className={`h-1.5 w-6 rounded-full transition-colors ${i < current ? "bg-emerald-400" : i === current ? "bg-sapBlue" : "bg-gray-200 dark:bg-slate-700"}`} />
          ))}
        </div>
      </div>
        ...
        {q.options.map((opt, idx) => {
          ...
          if (!showResult) {
            cls += selected === idx
              ...
              : "border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-sapBlue/50 hover:bg-gray-50 dark:hover:bg-slate-700";
          } else {
      ...
      <div className="flex gap-3">
        {!showResult ? (
          <button
            ...
            className="px-5 py-2 bg-sapBlue text-white rounded-xl text-sm font-semibold hover:bg-sapBlueDark disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            Valider
          </button>
        ) : (
          <button
            onClick={handleNext}
            className="px-5 py-2 bg-sapBlue text-white rounded-xl text-sm font-semibold hover:bg-sapBlueDark transition-colors"
          >
      ...
      {lesson.keyConcepts?.length > 0 && (
        <div>
          <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
            <span className="h-1.5 w-6 bg-sapBlue rounded-full" />
            Concepts clés
          </h4>
                ...
                {lesson.tcodes.map((t, i) => (
                  <tr key={i} className="border-t border-gray-100 dark:border-slate-700">
                    <td className="px-4 py-2.5">
                      <code className="bg-sapBlue/10 dark:bg-sapBlue/20 text-sapBlue dark:text-sapAccent px-2 py-0.5 rounded font-mono text-xs font-bold">
                        {t.code}
                      </code>
                    </td>
...
// ── Composant principal ──────────────────────────────────────────────────────
export default function CertificationFI() {
  ...
  const [completedLessons, setCompletedLessons] = useState(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem("fi_completed_lessons") || "[]"));
    } catch { return new Set(); }
  });
  const [showUpgradeModal, setShowUpgradeModal] = useState(false);

  const cert = fiCertification;
  const chapter = cert.chapters.find((c) => c.id === activeChapter);
  const lesson = chapter?.lessons.find((l) => l.id === activeLesson);

  // Persister les leçons complétées
  useEffect(() => {
    localStorage.setItem("fi_completed_lessons", JSON.stringify([...completedLessons]));
  }, [completedLessons]);
  ...
  return (
    <>
      <SEO
        title="Certification SAP FI — C_TS4FI_2023"
        description="Préparez la certification SAP Financial Accounting (C_TS4FI_2023) avec 7 chapitres, des quiz et un simulateur d'examen de 40 questions."
          ...
          >
            ...
            >
              <div className="text-center mb-5">
                <div className="h-14 w-14 bg-sapBlue/10 rounded-full flex items-center justify-center mx-auto mb-3">
                  <LockIcon />
                </div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Contenu Pro</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">
                  Ce chapitre est réservé aux membres Pro. Passez au plan Pro pour accéder à l'intégralité des chapitres et au simulateur d'examen.
                </p>
              </div>
              ...
              {!isAuthenticated ? (
                <div className="space-y-2">
                  <Link to="/register" className="block w-full text-center py-2.5 bg-sapBlue text-white rounded-xl font-semibold text-sm hover:bg-sapBlueDark transition-colors">
                    Créer un compte gratuit
                  </Link>
                  <p className="text-xs text-center text-slate-500">Déjà un compte ? <Link to="/login" className="text-sapBlue underline">Se connecter</Link></p>
                </div>
              ) : (
                <button onClick={handleUpgrade} className="w-full py-2.5 bg-sapBlue text-white rounded-xl font-semibold text-sm hover:bg-sapBlueDark transition-colors">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
            ...
            {!isPro && isAuthenticated && (
              <div className="mt-4 bg-white/10 border border-white/20 rounded-xl p-3 flex items-center justify-between gap-3">
                ...
                <button onClick={handleUpgrade} className="flex-shrink-0 px-4 py-1.5 bg-white text-sapBlue rounded-xl text-xs font-bold hover:bg-white/90 transition-colors">
                  Activer Pro (gratuit)
                </button>
              </div>
            )}
            {!isAuthenticated && (
              <div className="mt-4 bg-white/10 border border-white/20 rounded-xl p-3 flex items-center justify-between gap-3">
                <p className="text-sm text-white/90">
                  Connectez-vous pour suivre votre progression et accéder au contenu Pro.
                </p>
                <Link to="/register" className="flex-shrink-0 px-4 py-1.5 bg-white text-sapBlue rounded-xl text-xs font-bold hover:bg-white/90 transition-colors">
              ...
              <aside className="hidden lg:block w-72 flex-shrink-0 sticky top-20">
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 flex items-start gap-3 transition-colors
                            ${isActive ? "bg-sapBlue/10 dark:bg-sapBlue/20" : "hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                        >
                          <div className={`h-6 w-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold
                            ${isActive ? "bg-sapBlue text-white" : accessible ? "bg-gray-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300" : "bg-gray-100 dark:bg-slate-700 text-slate-400"}`}>
                            {accessible ? ch.number : <LockIcon />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={`text-xs font-semibold leading-tight ${isActive ? "text-sapBlue dark:text-sapAccent" : accessible ? "text-slate-800 dark:text-slate-200" : "text-slate-400 dark:text-slate-500"}`}>
                              {ch.title}
                            </p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-xs text-slate-400 dark:text-slate-500">
                                {chapterLessonsCompleted}/{ch.lessons.length} leçons
                              </span>
                              {ch.isPremium && !accessible && (
                                <span className="text-xs bg-sapBlue/10 text-sapBlue dark:text-sapAccent px-1.5 py-0.5 rounded font-semibold">Pro</span>
                              )}
                            </div>
                          </div>
                        </button>
                  ...
                  <div className="px-4 py-3 border-t border-gray-100 dark:border-slate-700">
                    {canAccess(true) ? (
                      <Link
                        to="/certifications/fi/examen"
                        className="block w-full text-center py-2.5 bg-sapBlue text-white rounded-xl text-sm font-semibold hover:bg-sapBlueDark transition-colors"
                      >
              ...
              <main className="flex-1 min-w-0">
                              ...
                              <button
                                ...
                                className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors flex items-center gap-2
                                  ${completedLessons.has(lesson.id)
                                    ...
                                    : "bg-sapBlue text-white hover:bg-sapBlueDark"}`}
                              >
                ...
                <div className="lg:hidden mt-4 bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 border-b border-gray-50 dark:border-slate-700/50 last:border-0 flex items-center gap-3 transition-colors
                          ${ch.id === activeChapter ? "bg-sapBlue/10" : "hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                      >
                        <span className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0
                          ${ch.id === activeChapter ? "bg-sapBlue text-white" : "bg-gray-100 dark:bg-slate-700 text-slate-500"}`}>
                          {accessible ? ch.number : <LockIcon />}
                        </span>
                        <span className={`text-sm font-medium ${ch.id === activeChapter ? "text-sapBlue dark:text-sapAccent" : accessible ? "text-slate-800 dark:text-slate-200" : "text-slate-400"}`}>
                          {ch.title}
                        </span>
                        {ch.isPremium && !accessible && (
                          <span className="ml-auto text-xs bg-sapBlue/10 text-sapBlue px-2 py-0.5 rounded font-semibold">Pro</span>
                        )}
                      </button>
                    );
                  })}
                  <div className="px-4 py-3">
                    {canAccess(true) ? (
                      <Link to="/certifications/fi/examen" className="block w-full text-center py-2.5 bg-sapBlue text-white rounded-xl text-sm font-semibold">
                        🎯 Simuler l'examen
                      </Link>
                    ) : (
