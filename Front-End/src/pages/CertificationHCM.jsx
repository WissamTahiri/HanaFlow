...
function QuizBlock({ quiz, onComplete }) {
  ...
  const handleConfirm = () => {
    ...
    setShowResult(true);
    setAnswers((prev) => [...prev, { correct: selected === q.correctIndex }]);
  };
    ...
    return (
      ...
      >
        ...
        <button
          ...
          className="mt-4 px-4 py-2 bg-purple-600 text-white rounded-xl text-sm font-semibold hover:bg-purple-700 transition-colors"
        >
          Recommencer le quiz
        </button>
      </motion.div>
    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-1">
          {quiz.map((_, i) => (
            <div key={i} className={`h-1.5 w-6 rounded-full transition-colors
              ${i < current ? "bg-purple-400" : i === current ? "bg-purple-600" : "bg-gray-200 dark:bg-slate-700"}`}
            />
          ))}
        </div>
      </div>
      ...
      <div className="space-y-2">
        {q.options.map((opt, idx) => {
          ...
          if (!showResult) {
            cls += selected === idx
              ? "border-purple-600 bg-purple-600/10 text-purple-700 dark:text-purple-400 font-medium"
              : "border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-purple-400/50 hover:bg-gray-50 dark:hover:bg-slate-700";
          } else {
            ...
            <button key={idx} className={cls} onClick={() => handleSelect(idx)} disabled={showResult}>
              <span className="flex items-center gap-2">
                <span className="h-5 w-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 text-xs font-bold"
                  style={{ borderColor: showResult && idx === q.correctIndex ? "#34d399" : showResult && idx === selected && idx !== q.correctIndex ? "#f87171" : selected === idx ? "#9333ea" : "#cbd5e1" }}>
      ...
      <div className="flex gap-3">
        {!showResult ? (
          <button onClick={handleConfirm} disabled={selected === null}
            className="px-5 py-2 bg-purple-600 text-white rounded-xl text-sm font-semibold hover:bg-purple-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
            Valider
          </button>
        ) : (
...
// ── LessonContent — rend le contenu markdown-like ────────────────────────────
function LessonContent({ lesson }) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("**") && line.endsWith("**") && !line.slice(2, -2).includes("**")) {
      elements.push(
        <p key={i} className="font-bold text-slate-900 dark:text-white text-sm mt-4 mb-1">{line.slice(2, -2)}</p>
      );
      ...
      elements.push(
        <ul key={i} className="list-disc list-inside space-y-1 pl-2 text-sm text-slate-700 dark:text-slate-300">
          {items.map((item, j) => (
      ...
      elements.push(
        <div key={i} className="overflow-auto rounded-xl border border-gray-200 dark:border-slate-700 text-sm my-2">
          <table className="w-full">
            {rows.map((row, ri) => (
              <tr key={ri} className={ri === 0 ? "bg-gray-50 dark:bg-slate-700/50 font-semibold" : "border-t border-gray-100 dark:border-slate-700"}>
                {row.map((cell, ci) => (
...
export default function CertificationHCM() {
  ...
  const [completedLessons, setCompletedLessons] = useState(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem("hcm_completed_lessons") || "[]"));
  ...
  useEffect(() => {
    localStorage.setItem("hcm_completed_lessons", JSON.stringify([...completedLessons]));
  }, [completedLessons]);
  ...
  const markLessonComplete = (lessonId) => {
    setCompletedLessons((prev) => {
      ...
      onLessonComplete("hcm", next.size);
      return next;
    });
  };
  ...
  return (
    <>
      <SEO
        title="Certification SAP HCM — C_THR81_2311"
        description="Préparez la certification SAP Human Capital Management (C_THR81_2311) avec 7 chapitres, des quiz interactifs et un simulateur d'examen complet."
        path="/certifications/hcm"
      />
          ...
          >
            ...
            >
              <div className="text-center mb-5">
                <div className="h-14 w-14 bg-purple-100 dark:bg-purple-900/30 rounded-full flex items-center justify-center mx-auto mb-3 text-purple-600">
                  <LockIcon />
                </div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Contenu Pro</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">
                  Ce chapitre est réservé aux membres Pro. Passez au plan Pro pour accéder à tous les chapitres HCM et au simulateur d'examen.
                </p>
              </div>
              <div className="space-y-2 mb-5">
                {[
                  "7 chapitres HCM complets (C_THR81_2311)",
                  ...
                  <div key={f} className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                    <span className="h-5 w-5 bg-purple-100 dark:bg-purple-900/40 rounded-full flex items-center justify-center text-purple-600 flex-shrink-0">
                      <CheckIcon />
                    </span>
                    {f}
                  </div>
              ...
              {!isAuthenticated ? (
                <div className="space-y-2">
                  <Link to="/register" className="block w-full text-center py-2.5 bg-purple-600 text-white rounded-xl font-semibold text-sm hover:bg-purple-700 transition-colors">
                    Créer un compte gratuit
                  </Link>
                  <p className="text-xs text-center text-slate-500">
                    Déjà un compte ? <Link to="/login" className="text-purple-600 underline">Se connecter</Link>
                  </p>
                </div>
              ) : (
                <button onClick={handleUpgrade} className="w-full py-2.5 bg-purple-600 text-white rounded-xl font-semibold text-sm hover:bg-purple-700 transition-colors">
      ...
      <div className="min-h-[calc(100vh-4.5rem)]">
        ...
        {/* Hero */}
        <div className="bg-gradient-to-br from-purple-900 via-purple-700 to-violet-500 text-white">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
            <nav className="flex items-center gap-2 text-xs text-white/60 mb-4">
              ...
              <span>/</span>
              <span className="text-white/90">SAP HCM</span>
            </nav>
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                <p className="text-xs font-mono text-white/60 mb-1">C_THR81_2311</p>
                <h1 className="text-2xl sm:text-3xl font-extrabold mb-2">SAP Human Capital Management</h1>
                <div className="flex flex-wrap gap-3 text-sm">
                  <span className="bg-white/20 px-3 py-1 rounded-full">{cert.questions} questions</span>
                  <span className="bg-white/20 px-3 py-1 rounded-full">{cert.duration}</span>
            ...
            {!isPro && isAuthenticated && (
              <div className="mt-4 bg-white/10 border border-white/20 rounded-xl p-3 flex items-center justify-between gap-3">
                ...
                <button onClick={handleUpgrade} className="flex-shrink-0 px-4 py-1.5 bg-white text-purple-700 rounded-xl text-xs font-bold hover:bg-white/90 transition-colors">
                  Activer Pro (gratuit)
                </button>
              </div>
            )}
            {!isAuthenticated && (
              <div className="mt-4 bg-white/10 border border-white/20 rounded-xl p-3 flex items-center justify-between gap-3">
                ...
                <Link to="/register" className="flex-shrink-0 px-4 py-1.5 bg-white text-purple-700 rounded-xl text-xs font-bold hover:bg-white/90 transition-colors">
        ...
        <div className="bg-gray-50 dark:bg-sapDark">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
            <div className="flex gap-6 items-start">
              ...
              {/* Sidebar */}
              <aside className="hidden lg:block w-72 flex-shrink-0 sticky top-[5rem]">
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 flex items-start gap-3 transition-colors
                            ${isActive ? "bg-purple-600/10 dark:bg-purple-600/20" : "hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                        >
                          <div className={`h-6 w-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold
                            ${isActive ? "bg-purple-600 text-white" : accessible ? "bg-gray-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300" : "bg-gray-100 dark:bg-slate-700 text-slate-400"}`}>
                            {accessible ? idx + 1 : <LockIcon />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={`text-xs font-semibold leading-tight ${isActive ? "text-purple-600 dark:text-purple-400" : accessible ? "text-slate-800 dark:text-slate-200" : "text-slate-400 dark:text-slate-500"}`}>
                              {ch.title}
                            </p>
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {ch.isPremium && !accessible && (
                                <span className="text-xs bg-purple-600/10 text-purple-600 dark:text-purple-400 px-1.5 py-0.5 rounded font-semibold">Pro</span>
                              )}
                            </div>
                          </div>
                        </button>
                  ...
                  <div className="px-4 py-3 border-t border-gray-100 dark:border-slate-700">
                    {canAccess(true) ? (
                      <Link to="/certifications/hcm/examen"
                        className="block w-full text-center py-2.5 bg-purple-600 text-white rounded-xl text-sm font-semibold hover:bg-purple-700 transition-colors">
              ...
              <main className="flex-1 min-w-0">
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 dark:border-slate-700">
                      <p className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider">
                        Chapitre {cert.chapters.indexOf(chapter) + 1}
                      </p>
                    ...
                    <div className="px-6 pt-4 flex flex-wrap gap-2 border-b border-gray-100 dark:border-slate-700 pb-3">
                      {chapter.lessons.map((l) => (
                        ...
                        >
                          {completedLessons.has(l.id) && <span className="text-purple-400"><CheckIcon /></span>}
                      ...
                      <AnimatePresence mode="wait">
                        {!showQuiz && lesson ? (
                          ...
                          >
                            ...
                            <QuizBlock
                              quiz={chapter.quiz}
                              onComplete={(s, t) => onQuizPass(Math.round((s / t) * 100))}
                            />
                          </motion.div>
                        ) : null}
                      </AnimatePresence>
                ...
                <div className="lg:hidden mt-4 bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 border-b border-gray-50 dark:border-slate-700/50 last:border-0 flex items-center gap-3 transition-colors
                          ${ch.id === activeChapter ? "bg-purple-600/10" : "hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                      >
                        <span className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0
                          ${ch.id === activeChapter ? "bg-purple-600 text-white" : "bg-gray-100 dark:bg-slate-700 text-slate-500"}`}>
                          {accessible ? idx + 1 : <LockIcon />}
                        </span>
                        <span className={`text-sm font-medium ${ch.id === activeChapter ? "text-purple-600 dark:text-purple-400" : accessible ? "text-slate-800 dark:text-slate-200" : "text-slate-400"}`}>
                          {ch.title}
                        </span>
                        {ch.isPremium && !accessible && (
                          <span className="ml-auto text-xs bg-purple-600/10 text-purple-600 px-2 py-0.5 rounded font-semibold">Pro</span>
                        )}
                      </button>
                    );
                  })}
                  <div className="px-4 py-3">
                    {canAccess(true) ? (
                      <Link to="/certifications/hcm/examen" className="block w-full text-center py-2.5 bg-purple-600 text-white rounded-xl text-sm font-semibold">
                        🎯 Simuler l'examen
                      </Link>
                    ) : (
