    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-8 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
          Simulateur d'examen — {sdCertification.code}
        </h2>
        <p className="text-slate-600 dark:text-slate-400 mb-6">{sdCertification.shortName}</p>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            { label: "Questions", value: sdMockExamQuestions.length },
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 mb-6 text-left">
          ...
          <ul className="text-sm text-amber-700 dark:text-amber-400 space-y-1">
            <li>• {sdMockExamQuestions.length} questions à choix multiple, 4 options chacune</li>
        ...
        <Link to="/certifications/sd" className="block mt-3 text-sm text-slate-500 hover:text-purple-600 transition-colors">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button onClick={() => setReviewIndex((i) => Math.min(questions.length - 1, i + 1))} disabled={reviewIndex === questions.length - 1} className="px-4 py-2 bg-purple-600 text-white rounded-xl text-sm font-semibold disabled:opacity-40">Suivante →</button>
          </div>
        </div>
      </motion.div>
    );
      ...
      {passed && (
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">Certificat PDF personnalisé — SAP SD (C_TS4SD_2023)</p>
          </div>
          <CertificateDownloadButton
            userName={user?.name}
            moduleCode="C_TS4SD_2023"
...
export default function ExamSimulatorSD() {
  ...
  const [answers, setAnswers] = useState(new Array(sdMockExamQuestions.length).fill(null));
      ...
      <>
        ...
        <div className="min-h-[calc(100vh-4rem)] bg-gray-50 dark:bg-sapDark flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-8 text-center max-w-md">
            ...
            <Link to="/certifications/sd" className="block w-full py-2.5 bg-purple-600 text-white rounded-xl font-semibold text-sm hover:bg-purple-700 transition-colors">
              Retour — Activer Pro
            </Link>
          </div>
        </div>
  ...
  const handleAnswer = (idx) => { setAnswers((prev) => { const next = [...prev]; next[currentQ] = idx; return next; }); };
  const handleSubmit = () => {
    clearInterval(timerRef.current);
    const score = answers.filter((a, i) => a === sdMockExamQuestions[i].correctIndex).length;
    onExamComplete("sd", Math.round((score / sdMockExamQuestions.length) * 100) >= 65);
    setPhase("results");
  };
  const toggleFlag = () => {
  ...
  const q = sdMockExamQuestions[currentQ];
    ...
    <>
      ...
      <div className="min-h-[calc(100vh-4rem)] bg-gray-50 dark:bg-sapDark">
          ...
          <div className="sticky top-16 z-30 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-4 py-2">
            <div className="max-w-4xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Q {currentQ + 1}/{sdMockExamQuestions.length}</span>
                <span className="text-xs text-slate-400">{answeredCount}/{sdMockExamQuestions.length} répondues</span>
              </div>
            ...
            <div className="max-w-4xl mx-auto mt-2">
              <div className="h-1 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                <div className="h-full bg-purple-600 rounded-full transition-all" style={{ width: `${(answeredCount / sdMockExamQuestions.length) * 100}%` }} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            <div className="grid lg:grid-cols-[1fr_auto] gap-5">
              ...
              >
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100 dark:border-slate-700">
                  ...
                  <button onClick={() => setCurrentQ((c) => Math.min(sdMockExamQuestions.length - 1, c + 1))} disabled={currentQ === sdMockExamQuestions.length - 1}
          ...
          {phase === "results" && <ResultsScreen answers={answers} questions={sdMockExamQuestions} timeUsed={timeUsed} />}
        </div>
      </div>
    </>
  );
