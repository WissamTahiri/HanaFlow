...
const MermaidBlock = ({ title, code }) => (
  <div className="mt-5 border border-cyan-500/20 rounded-2xl p-4 sm:p-5 bg-slate-900">
    ...
    <pre className="text-[11px] sm:text-xs text-slate-100 overflow-x-auto leading-relaxed">
      {code}
    </pre>
  </div>
);
...
const faqItems = [
  {
    q: "Quelle est la différence entre P2P et O2C ?",
    a: "P2P (Procure-to-Pay) décrit les achats — comment l'argent sort de l'entreprise via les fournisseurs. O2C (Order-to-Cash) décrit les ventes — comment le chiffre d'affaires entre dans l'entreprise via les clients. Les deux cycles se recoupent dans FI/CO lors de la clôture.",
  },
  {
    q: "Pourquoi R2R est-il important pour un consultant SAP ?",
    a: "R2R est le cycle qui consolide tous les flux (P2P, O2C, paie, actifs) en états financiers. Comprendre R2R permet de faire le lien entre les opérations quotidiennes et le bilan/P&L — une compétence clé pour les consultants FI, CO et les chefs de projet.",
  },
  {
    q: "Qu'est-ce que le 3-way match dans P2P ?",
    a: "Le 3-way match est la vérification automatique de cohérence entre le Purchase Order (PO), le Goods Receipt (GR) et la facture fournisseur (IR). SAP bloque le paiement si les 3 documents ne correspondent pas, protégeant l'entreprise contre les paiements frauduleux ou erronés.",
  },
  {
    q: "Quels modules SAP sont impliqués dans O2C ?",
    a: "Le cycle O2C implique principalement SD (création de commande, livraison, facturation) et FI (comptabilisation des revenus, postes clients AR, paiements). CO est aussi impliqué pour l'analyse de marge (CO-PA) lors de la facturation.",
  },
  {
    q: "Comment ces processus sont-ils liés dans S/4HANA ?",
    a: "Dans S/4HANA, l'Universal Journal (ACDOCA) unifie toutes les écritures FI/CO générées par P2P, O2C et R2R en une source unique. Cela élimine les réconciliations inter-modules et permet un reporting temps réel sur tous les cycles.",
  },
];
...
const ProcessSteps = ({ steps, accentColor }) => (
  <div className="space-y-3">
    {steps.map((s) => (
      <div key={s.step} className="flex gap-4 items-start">
        <div
          className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white ${accentColor}`}
        >
          {s.step}
        </div>
          ...
          {s.tcodes && s.tcodes.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1.5">
              {s.tcodes.map((t) => (
                <span
                  key={t}
                  className="px-2 py-0.5 rounded text-[10px] font-mono font-semibold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600"
                >
                  {t}
                </span>
              ))}
        ...
        {faqItems.map((item, i) => (
          <div key={i} className="border border-cyan-500/15 rounded-2xl overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 text-left text-sm font-semibold hover:bg-cyan-50 dark:hover:bg-cyan-900/20 transition-colors"
            >
              <span>{item.q}</span>
  ...
  >
    ...
    <Section title="Vue d'ensemble des 3 cycles">
      <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 mb-5 leading-relaxed">
        Les processus P2P, O2C et R2R structurent la majorité des flux d'une
        entreprise. Comprendre ces cycles avant d'étudier les modules SAP individuels
        permet de voir comment FI, MM, SD et CO s'articulent dans un vrai projet.
      </p>
        ...
        {[
          {
            ...
            subtitle: "Achats",
            desc: "De l'expression du besoin au paiement fournisseur. Modules : MM + FI",
          ...
          },
          {
            ...
            title: "Order-to-Cash",
            subtitle: "Ventes",
            desc: "De la commande client à l'encaissement. Modules : SD + FI",
          ...
          },
          {
            ...
            subtitle: "Finance",
            desc: "De l'enregistrement comptable aux états financiers. Modules : FI + CO",
            color: "bg-blue-500/10 text-blue-700 dark:text-blue-300 border-blue-500/30",
          },
        ].map((c) => (
          <div
            key={c.code}
            className="rounded-2xl border p-5 bg-gray-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700"
          >
            <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-bold border mb-3 ${c.color}`}>
              {c.code}
            </span>
    ...
    <Section title="1. P2P – Procure-to-Pay (Achats)">
      <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 mb-5 leading-relaxed">
        P2P couvre toutes les étapes de l'expression de besoin jusqu'au paiement
        fournisseur. Il contrôle comment l'argent sort de l'entreprise et mobilise
        principalement les modules MM et FI.
      </p>
    ...
    <Section title="2. O2C – Order-to-Cash (Ventes)">
      <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 mb-5 leading-relaxed">
        O2C décrit le flux depuis la commande client jusqu'à l'encaissement. Il
        contrôle comment le chiffre d'affaires entre dans l'entreprise, via les
        modules SD et FI.
      </p>
    ...
    <Section title="3. R2R – Record-to-Report (Finance)">
      <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 mb-5 leading-relaxed">
        R2R regroupe tous les enregistrements comptables (P2P, O2C, paie…), leur
        consolidation et la production des états financiers. Il explique comment les
        chiffres sont produits dans FI et CO.
      </p>
      <ProcessSteps steps={r2rSteps} accentColor="bg-blue-500" />
      <MermaidBlock title="Schéma R2R" code={r2rMermaid} />
    </Section>

    {/* Lien avec les modules HanaFlow */}
    <Section title="4. Lien avec les modules HanaFlow">
      <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 mb-5 leading-relaxed">
        Chaque processus end-to-end traverse plusieurs modules SAP. Savoir raconter
        ces histoires de bout en bout — en citant les t-codes, les écritures générées
        et les modules impliqués — est ce qui différencie un consultant junior
        compétent d'un simple utilisateur SAP.
      </p>
      <div className="grid sm:grid-cols-3 gap-4">
        {[
          {
            cycle: "P2P",
            modules: ["MM (PR, PO, GR)", "FI (IR, paiement)", "CO (imputation)"],
            color: "border-orange-500/20 bg-orange-50 dark:bg-orange-900/10",
            badge: "bg-orange-500/10 text-orange-700 dark:text-orange-300 border-orange-500/30",
          },
          {
            cycle: "O2C",
            modules: ["SD (Sales Order, Delivery, Billing)", "FI (revenus, AR)", "CO (CO-PA, marge)"],
            color: "border-purple-500/20 bg-purple-50 dark:bg-purple-900/10",
            badge: "bg-purple-500/10 text-purple-700 dark:text-purple-300 border-purple-500/30",
          },
          {
            cycle: "R2R",
            modules: ["FI (clôture, états)", "CO (reporting gestion)", "BI/Fiori (analytique)"],
            color: "border-blue-500/20 bg-blue-50 dark:bg-blue-900/10",
          ...
          <div key={m.cycle} className={`rounded-2xl border p-4 ${m.color}`}>
            <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-bold border mb-3 ${m.badge}`}>
              {m.cycle}
            </span>
