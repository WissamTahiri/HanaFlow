    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
        ...
        <div className="flex gap-1">
          {quiz.map((_, i) => (
            <div key={i} className={`h-1.5 w-6 rounded-full transition-colors ${i < current ? "bg-emerald-400" : i === current ? "bg-emerald-600" : "bg-gray-200 dark:bg-slate-700"}`} />
          ))}
        </div>
      </div>
      ...
      <div className="space-y-2">
        {q.options.map((opt, idx) => {
          ...
          if (!showResult) {
            cls += selected === idx
              ? "border-emerald-600 bg-emerald-600/10 text-emerald-700 dark:text-emerald-400 font-medium"
              : "border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-emerald-400/50 hover:bg-gray-50 dark:hover:bg-slate-700";
          } else {
            ...
            <button key={idx} className={cls} onClick={() => handleSelect(idx)} disabled={showResult}>
              <span className="flex items-center gap-2">
                <span className="h-5 w-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 text-xs font-bold"
                  style={{ borderColor: showResult && idx === q.correctIndex ? "#34d399" : showResult && idx === selected && idx !== q.correctIndex ? "#f87171" : selected === idx ? "#059669" : "#cbd5e1" }}>
      ...
      <div className="flex gap-3">
        {!showResult ? (
          <button onClick={handleConfirm} disabled={selected === null}
            className="px-5 py-2 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
            Valider
          </button>
        ) : (
      ...
      {lesson.keyConcepts?.length > 0 && (
        <div>
          <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
            <span className="h-1.5 w-6 bg-emerald-600 rounded-full" />
            Concepts clés
          </h4>
          <div className="grid gap-2">
            {lesson.keyConcepts.map((c, i) => (
              <div key={i} className="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl px-4 py-3 border border-emerald-100 dark:border-emerald-800">
                <span className="font-semibold text-sm text-emerald-900 dark:text-emerald-200">{c.term}</span>
                ...
                {lesson.tcodes.map((t, i) => (
                  <tr key={i} className="border-t border-gray-100 dark:border-slate-700">
                    <td className="px-4 py-2.5">
                      <code className="bg-emerald-600/10 dark:bg-emerald-600/20 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded font-mono text-xs font-bold">
                        {t.code}
                      </code>
                    </td>
...
export default function CertificationMM() {
  ...
  const [completedLessons, setCompletedLessons] = useState(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem("mm_completed_lessons") || "[]"));
    } catch { return new Set(); }
  });
  const [showUpgradeModal, setShowUpgradeModal] = useState(false);

  const cert = mmCertification;
  ...
  return (
    <>
      <SEO
        title="Certification SAP MM — C_TS4MM_2023"
        description="Préparez la certification SAP Materials Management (C_TS4MM_2023) avec 7 chapitres, des quiz et un simulateur d'examen de 40 questions."
          ...
          >
            ...
            >
              <div className="text-center mb-5">
                <div className="h-14 w-14 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center mx-auto mb-3 text-emerald-600">
                  <LockIcon />
                </div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Contenu Pro</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">
                  Ce chapitre est réservé aux membres Pro. Passez au plan Pro pour accéder à tous les chapitres MM et au simulateur d'examen.
                </p>
              </div>
              <div className="space-y-2 mb-5">
                {[
                  "7 chapitres MM complets (C_TS4MM_2023)",
              ...
              {!isAuthenticated ? (
                <div className="space-y-2">
                  <Link to="/register" className="block w-full text-center py-2.5 bg-emerald-600 text-white rounded-xl font-semibold text-sm hover:bg-emerald-700 transition-colors">
                    Créer un compte gratuit
                  </Link>
                  <p className="text-xs text-center text-slate-500">
                    Déjà un compte ? <Link to="/login" className="text-emerald-600 underline">Se connecter</Link>
                  </p>
                </div>
              ) : (
                <button onClick={handleUpgrade} className="w-full py-2.5 bg-emerald-600 text-white rounded-xl font-semibold text-sm hover:bg-emerald-700 transition-colors">
      ...
      <div className="min-h-[calc(100vh-4rem)]">
        {/* Hero */}
        <div className="bg-gradient-to-br from-emerald-900 via-emerald-700 to-emerald-500 text-white">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
            <nav className="flex items-center gap-2 text-xs text-white/60 mb-4">
              ...
              <span>/</span>
              <span className="text-white/90">SAP MM</span>
            </nav>
            ...
            {!isPro && isAuthenticated && (
              <div className="mt-4 bg-white/10 border border-white/20 rounded-xl p-3 flex items-center justify-between gap-3">
                ...
                <button onClick={handleUpgrade} className="flex-shrink-0 px-4 py-1.5 bg-white text-emerald-700 rounded-xl text-xs font-bold hover:bg-white/90 transition-colors">
                  Activer Pro (gratuit)
                </button>
              </div>
            )}
            {!isAuthenticated && (
              <div className="mt-4 bg-white/10 border border-white/20 rounded-xl p-3 flex items-center justify-between gap-3">
                ...
                <Link to="/register" className="flex-shrink-0 px-4 py-1.5 bg-white text-emerald-700 rounded-xl text-xs font-bold hover:bg-white/90 transition-colors">
              ...
              <aside className="hidden lg:block w-72 flex-shrink-0 sticky top-20">
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 flex items-start gap-3 transition-colors
                            ${isActive ? "bg-emerald-600/10 dark:bg-emerald-600/20" : "hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                        >
                          <div className={`h-6 w-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold
                            ${isActive ? "bg-emerald-600 text-white" : accessible ? "bg-gray-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300" : "bg-gray-100 dark:bg-slate-700 text-slate-400"}`}>
                            {accessible ? ch.number : <LockIcon />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={`text-xs font-semibold leading-tight ${isActive ? "text-emerald-600 dark:text-emerald-400" : accessible ? "text-slate-800 dark:text-slate-200" : "text-slate-400 dark:text-slate-500"}`}>
                              {ch.title}
                            </p>
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {ch.isPremium && !accessible && (
                                <span className="text-xs bg-emerald-600/10 text-emerald-600 dark:text-emerald-400 px-1.5 py-0.5 rounded font-semibold">Pro</span>
                              )}
                            </div>
                          </div>
                        </button>
                  ...
                  <div className="px-4 py-3 border-t border-gray-100 dark:border-slate-700">
                    {canAccess(true) ? (
                      <Link to="/certifications/mm/examen"
                        className="block w-full text-center py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-colors">
              ...
              <main className="flex-1 min-w-0">
                ...
                <div className="lg:hidden mt-4 bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 border-b border-gray-50 dark:border-slate-700/50 last:border-0 flex items-center gap-3 transition-colors
                          ${ch.id === activeChapter ? "bg-emerald-600/10" : "hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                      >
                        <span className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0
                          ${ch.id === activeChapter ? "bg-emerald-600 text-white" : "bg-gray-100 dark:bg-slate-700 text-slate-500"}`}>
                          {accessible ? ch.number : <LockIcon />}
                        </span>
                        <span className={`text-sm font-medium ${ch.id === activeChapter ? "text-emerald-600 dark:text-emerald-400" : accessible ? "text-slate-800 dark:text-slate-200" : "text-slate-400"}`}>
                          {ch.title}
                        </span>
                        {ch.isPremium && !accessible && (
                          <span className="ml-auto text-xs bg-emerald-600/10 text-emerald-600 px-2 py-0.5 rounded font-semibold">Pro</span>
                        )}
                      </button>
                    );
                  })}
                  <div className="px-4 py-3">
                    {canAccess(true) ? (
                      <Link to="/certifications/mm/examen" className="block w-full text-center py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-semibold">
                        🎯 Simuler l'examen
                      </Link>
                    ) : (
