      ...
      {passed && (
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">Certificat PDF personnalisé — SAP MM (C_TS4MM_2023)</p>
          </div>
          <CertificateDownloadButton
            userName={user?.name}
            moduleCode="C_TS4MM_2023"
...
export default function ExamSimulatorMM() {
  ...
  const handleSubmit = () => {
    ...
    const score = answers.filter((a, i) => a === mmMockExamQuestions[i].correctIndex).length;
    onExamComplete("mm", Math.round((score / mmMockExamQuestions.length) * 100) >= 65);
    setPhase("results");
  };
  const toggleFlag = () => {
