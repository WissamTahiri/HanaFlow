    ...
    >
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-8 text-center">
        <div className="h-16 w-16 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
          <svg className="w-8 h-8 text-sapBlue" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        </div>
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
          Simulateur d'examen — {fiCertification.code}
        </h2>
        <p className="text-slate-600 dark:text-slate-400 mb-6">
          {fiCertification.shortName}
        </p>

        {/* Règles de l'examen */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            { label: "Questions", value: fiMockExamQuestions.length },
            ...
            <div key={item.label} className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3">
              <p className="text-xl font-black text-sapBlue">{item.value}</p>
        ...
        <button
          onClick={onStart}
          className="w-full py-3 bg-sapBlue text-white rounded-xl font-bold text-base hover:bg-sapBlueDark transition-colors"
        >
          Démarrer l'examen
        </button>
        <Link
          to="/certifications/fi"
          className="block mt-3 text-sm text-slate-500 hover:text-sapBlue transition-colors"
        >
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setReviewMode(false)} className="text-sm text-sapBlue hover:underline">Retour aux résultats</button>
          </div>
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button onClick={() => setReviewIndex((i) => Math.min(questions.length - 1, i + 1))} disabled={reviewIndex === questions.length - 1} className="px-4 py-2 bg-sapBlue text-white rounded-xl text-sm font-semibold disabled:opacity-40">Suivante →</button>
          </div>
        </div>
      </motion.div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-5">
      ...
      <div className={`rounded-2xl p-8 text-center border-2 ${passed ? "border-emerald-400 bg-emerald-50 dark:bg-emerald-900/20" : "border-orange-400 bg-orange-50 dark:bg-orange-900/20"}`}>
        <div className={`text-6xl font-black mb-2 ${passed ? "text-emerald-600 dark:text-emerald-400" : "text-orange-500"}`}>
          {pct}%
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Link
          to="/certifications/fi"
          className="flex-1 py-2.5 bg-sapBlue text-white rounded-xl text-sm font-semibold text-center hover:bg-sapBlueDark transition-colors"
        >
          Retour aux chapitres
        </Link>
      </div>
...
export default function ExamSimulator() {
  ...
  const [answers, setAnswers] = useState(new Array(fiMockExamQuestions.length).fill(null));
  const [timeLeft, setTimeLeft] = useState(EXAM_DURATION);
  const [timeUsed, setTimeUsed] = useState(0);
  const [flagged, setFlagged] = useState(new Set());
  const timerRef = useRef(null);

  // Vérification accès Pro
  if (!canAccess(true)) {
    return (
      <>
        <SEO title="Simulateur d'examen SAP FI" description="Accès Pro requis" path="/certifications/fi/examen" />
        <div className="min-h-[calc(100vh-4rem)] bg-gray-50 dark:bg-sapDark flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-8 text-center max-w-md">
            <div className="h-14 w-14 bg-sapBlue/10 rounded-full flex items-center justify-center mx-auto mb-4">
              <svg className="w-7 h-7 text-sapBlue" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <Link to="/certifications/fi" className="block w-full py-2.5 bg-sapBlue text-white rounded-xl font-semibold text-sm hover:bg-sapBlueDark transition-colors">
              Retour — Activer Pro
            </Link>
          </div>
        </div>
  ...
  const q = fiMockExamQuestions[currentQ];

  const timerColor = timeLeft < 300 ? "text-red-500" : timeLeft < 900 ? "text-orange-500" : "text-slate-700 dark:text-slate-200";

  return (
    <>
      <SEO title="Simulateur d'examen SAP FI — C_TS4FI_2023" description="Simulateur d'examen 40 questions format réel SAP FI." path="/certifications/fi/examen" />
          ...
          <div className="sticky top-16 z-30 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-4 py-2">
            <div className="max-w-4xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                  Q {currentQ + 1}/{fiMockExamQuestions.length}
                </span>
                <span className="text-xs text-slate-400">{answeredCount}/{fiMockExamQuestions.length} répondues</span>
              </div>
              {/* Timer */}
              ...
              <button
                ...
                className="px-4 py-1.5 bg-sapBlue text-white rounded-xl text-sm font-semibold hover:bg-sapBlueDark transition-colors"
              >
                Terminer l'examen
              </button>
            </div>
            ...
            <div className="max-w-4xl mx-auto mt-2">
              <div className="h-1 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                <div className="h-full bg-sapBlue rounded-full transition-all" style={{ width: `${(answeredCount / fiMockExamQuestions.length) * 100}%` }} />
              </div>
            </div>
          </div>
        )}
          ...
          {phase === "exam" && (
            <div className="grid lg:grid-cols-[1fr_auto] gap-5">
              ...
              >
                {/* Chapitre + flag */}
                <div className="flex items-center justify-between mb-4">
                  <span className="text-xs font-semibold text-sapBlue dark:text-sapAccent bg-sapBlue/10 dark:bg-sapBlue/20 px-2.5 py-1 rounded-lg">
                    {q.chapter}
                  </span>
                  <button
                    onClick={toggleFlag}
                    title={flagged.has(currentQ) ? "Retirer le marquage" : "Marquer pour révision"}
                ...
                <p className="text-base font-semibold text-slate-900 dark:text-white leading-relaxed mb-5">
                  {q.question}
                </p>
                    ...
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${
                        answers[currentQ] === idx
                          ? "border-sapBlue bg-sapBlue/10 dark:bg-sapBlue/20 text-sapBlue dark:text-sapAccent font-semibold"
                          : "border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-sapBlue/40 hover:bg-gray-50 dark:hover:bg-slate-700"
                      }`}
                    >
                      <span className="inline-flex items-center gap-2">
                        <span className={`h-6 w-6 rounded-full border-2 flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                          answers[currentQ] === idx ? "border-sapBlue bg-sapBlue text-white" : "border-gray-300 dark:border-slate-600"
                        }`}>
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100 dark:border-slate-700">
                  ...
                  <span className="text-xs text-slate-400">
                    {answers[currentQ] !== null ? "✓ Répondue" : "Non répondue"}
                  </span>
                  <button
                    onClick={() => setCurrentQ((c) => Math.min(fiMockExamQuestions.length - 1, c + 1))}
                    disabled={currentQ === fiMockExamQuestions.length - 1}
                    className="px-4 py-2 bg-sapBlue text-white rounded-xl text-sm font-semibold hover:bg-sapBlueDark disabled:opacity-40 transition-colors"
                  >
                    Suivante →
                  </button>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-4 sticky top-32">
                  ...
                  <div className="mt-3 space-y-1.5">
                    <div className="flex items-center gap-2 text-xs text-slate-500">
                      <div className="h-3 w-3 rounded bg-emerald-100 dark:bg-emerald-900/30 border border-emerald-200" />
                      Répondue
                    </div>
                    <div className="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500">
                      <div className="h-3 w-3 rounded bg-gray-100 dark:bg-slate-700" />
                      Non répondue
                    </div>
                  </div>
                </div>
