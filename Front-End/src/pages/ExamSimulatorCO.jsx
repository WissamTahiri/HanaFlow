    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-8 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
          Simulateur d'examen — {coCertification.code}
        </h2>
        <p className="text-slate-600 dark:text-slate-400 mb-6">{coCertification.shortName}</p>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            { label: "Questions", value: coMockExamQuestions.length },
            ...
            <div key={item.label} className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3">
              <p className="text-xl font-black text-indigo-600">{item.value}</p>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 mb-6 text-left">
          ...
          <ul className="text-sm text-amber-700 dark:text-amber-400 space-y-1">
            <li>• {coMockExamQuestions.length} questions à choix multiple, 4 options chacune</li>
        ...
        <button onClick={onStart} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold text-base hover:bg-indigo-700 transition-colors">
          Démarrer l'examen
        </button>
        <Link to="/certifications/co" className="block mt-3 text-sm text-slate-500 hover:text-indigo-600 transition-colors">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setReviewMode(false)} className="text-sm text-indigo-600 hover:underline">Retour aux résultats</button>
          </div>
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button onClick={() => setReviewIndex((i) => Math.min(questions.length - 1, i + 1))} disabled={reviewIndex === questions.length - 1} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-semibold disabled:opacity-40">Suivante →</button>
          </div>
        </div>
      </motion.div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Link to="/certifications/co"
          className="flex-1 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-semibold text-center hover:bg-indigo-700 transition-colors">
          Retour aux chapitres
        </Link>
      </div>
      ...
      {passed && (
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">Certificat PDF personnalisé — SAP CO (C_TS4CO_2023)</p>
          </div>
          <CertificateDownloadButton
            userName={user?.name}
            moduleCode="C_TS4CO_2023"
...
export default function ExamSimulatorCO() {
  ...
  const [answers, setAnswers] = useState(new Array(coMockExamQuestions.length).fill(null));
  ...
  if (!canAccess(true)) {
    return (
      <>
        <SEO title="Simulateur d'examen SAP CO" description="Accès Pro requis" path="/certifications/co/examen" />
        <div className="min-h-[calc(100vh-4rem)] bg-gray-50 dark:bg-slate-950 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-8 text-center max-w-md">
            <div className="h-14 w-14 bg-indigo-600/10 rounded-full flex items-center justify-center mx-auto mb-4">
            ...
            <Link to="/certifications/co" className="block w-full py-2.5 bg-indigo-600 text-white rounded-xl font-semibold text-sm hover:bg-indigo-700 transition-colors">
              Retour — Activer Pro
            </Link>
          </div>
        </div>
  ...
  const q = coMockExamQuestions[currentQ];
  const timerColor = timeLeft < 300 ? "text-red-500" : timeLeft < 900 ? "text-orange-500" : "text-slate-700 dark:text-slate-200";

  return (
    <>
      <SEO title="Simulateur d'examen SAP CO — C_TS4CO_2023" description="Simulateur d'examen 40 questions format réel SAP CO Controlling." path="/certifications/co/examen" />
            ...
            <div className="max-w-4xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                  Q {currentQ + 1}/{coMockExamQuestions.length}
                </span>
                <span className="text-xs text-slate-400">{answeredCount}/{coMockExamQuestions.length} répondues</span>
              </div>
              ...
              <button onClick={handleSubmit} className="px-4 py-1.5 bg-indigo-600 text-white rounded-xl text-sm font-semibold hover:bg-indigo-700 transition-colors">
                Terminer l'examen
              </button>
            </div>
            <div className="max-w-4xl mx-auto mt-2">
              <div className="h-1 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                <div className="h-full bg-indigo-600 rounded-full transition-all" style={{ width: `${(answeredCount / coMockExamQuestions.length) * 100}%` }} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          {phase === "exam" && (
            <div className="grid lg:grid-cols-[1fr_auto] gap-5">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-600/10 dark:bg-indigo-600/20 px-2.5 py-1 rounded-lg">
                    {q.chapter}
                  </span>
                    ...
                    <button key={idx} onClick={() => handleAnswer(idx)}
                      className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${
                        answers[currentQ] === idx
                          ? "border-indigo-600 bg-indigo-600/10 dark:bg-indigo-600/20 text-indigo-700 dark:text-indigo-400 font-semibold"
                          : "border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-indigo-400/40 hover:bg-gray-50 dark:hover:bg-slate-700"
                      }`}>
                      <span className="inline-flex items-center gap-2">
                        <span className={`h-6 w-6 rounded-full border-2 flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                          answers[currentQ] === idx ? "border-indigo-600 bg-indigo-600 text-white" : "border-gray-300 dark:border-slate-600"
                        }`}>
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100 dark:border-slate-700">
                  ...
                  <button onClick={() => setCurrentQ((c) => Math.min(coMockExamQuestions.length - 1, c + 1))} disabled={currentQ === coMockExamQuestions.length - 1}
          ...
          {phase === "results" && <ResultsScreen answers={answers} questions={coMockExamQuestions} timeUsed={timeUsed} />}
        </div>
      </div>
    </>
  );
