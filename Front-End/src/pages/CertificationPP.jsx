    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-1">
          {quiz.map((_, i) => (
            <div key={i} className={`h-1.5 w-6 rounded-full transition-colors
              ${i < current ? "bg-rose-400" : i === current ? "bg-rose-600" : "bg-gray-200 dark:bg-slate-700"}`}
            />
          ))}
        </div>
      </div>
      ...
      <div className="space-y-2">
        {q.options.map((opt, idx) => {
          ...
          if (!showResult) {
            cls += selected === idx
              ? "border-rose-600 bg-rose-600/10 text-rose-700 dark:text-rose-400 font-medium"
              : "border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-rose-400/50 hover:bg-gray-50 dark:hover:bg-slate-700";
          } else {
            ...
            <button key={idx} className={cls} onClick={() => handleSelect(idx)} disabled={showResult}>
              <span className="flex items-center gap-2">
                <span className="h-5 w-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 text-xs font-bold"
                  style={{ borderColor: showResult && idx === q.correctIndex ? "#34d399" : showResult && idx === selected && idx !== q.correctIndex ? "#f87171" : selected === idx ? "#e11d48" : "#cbd5e1" }}>
      ...
      <div className="flex gap-3">
        {!showResult ? (
          <button onClick={handleConfirm} disabled={selected === null}
            className="px-5 py-2 bg-rose-600 text-white rounded-xl text-sm font-semibold hover:bg-rose-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
            Valider
          </button>
        ) : (
          <button onClick={handleNext}
            className="px-5 py-2 bg-rose-600 text-white rounded-xl text-sm font-semibold hover:bg-rose-700 transition-colors">
...
// ── Composant principal ───────────────────────────────────────────────────────
export default function CertificationPP() {
  const { isPro, canAccess } = useSubscription();
  const navigate = useNavigate();
  const { isAuthenticated } = useAuth();
  const { onLessonComplete, onQuizPass } = useGamification();

  const cert = ppCertification;

  const [activeChapter, setActiveChapter] = useState("ch1");
  const [activeLesson, setActiveLesson] = useState("ch1-l1");
  const [showQuiz, setShowQuiz] = useState(false);
  const [completedLessons, setCompletedLessons] = useState(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem("pp_completed_lessons") || "[]"));
  ...
  useEffect(() => {
    localStorage.setItem("pp_completed_lessons", JSON.stringify([...completedLessons]));
  }, [completedLessons]);
  ...
  return (
    <>
      <SEO
        title="Certification SAP PP — C_TS422_2023"
        description="Préparez la certification SAP Production Planning (C_TS422_2023) avec 7 chapitres, des quiz interactifs et un simulateur d'examen complet."
        path="/certifications/pp"
      />
          ...
          >
            ...
            >
              <div className="text-center mb-5">
                <div className="h-14 w-14 bg-rose-100 dark:bg-rose-900/30 rounded-full flex items-center justify-center mx-auto mb-3 text-rose-600">
                  <LockIcon />
                </div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Contenu Pro</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">
                  Ce chapitre est réservé aux membres Pro. Passez au plan Pro pour accéder à tous les chapitres PP et au simulateur d'examen.
                </p>
              </div>
              <div className="space-y-2 mb-5">
                {[
                  "7 chapitres PP complets (C_TS422_2023)",
                  ...
                  <div key={f} className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                    <span className="h-5 w-5 bg-rose-100 dark:bg-rose-900/40 rounded-full flex items-center justify-center text-rose-600 flex-shrink-0">
                      <CheckIcon />
                    </span>
                    {f}
                  </div>
              ...
              {!isAuthenticated ? (
                <div className="space-y-2">
                  <Link to="/register" className="block w-full text-center py-2.5 bg-rose-600 text-white rounded-xl font-semibold text-sm hover:bg-rose-700 transition-colors">
                    Créer un compte gratuit
                  </Link>
                  <p className="text-xs text-center text-slate-500">
                    Déjà un compte ? <Link to="/login" className="text-rose-600 underline">Se connecter</Link>
                  </p>
                </div>
              ) : (
                <button onClick={handleUpgrade} className="w-full py-2.5 bg-rose-600 text-white rounded-xl font-semibold text-sm hover:bg-rose-700 transition-colors">
      ...
      <div className="min-h-[calc(100vh-4.5rem)]">
        ...
        {/* Hero */}
        <div className="bg-gradient-to-br from-rose-900 via-rose-700 to-pink-500 text-white">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
            <nav className="flex items-center gap-2 text-xs text-white/60 mb-4">
              ...
              <span>/</span>
              <span className="text-white/90">SAP PP</span>
            </nav>
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                <p className="text-xs font-mono text-white/60 mb-1">C_TS422_2023</p>
                <h1 className="text-2xl sm:text-3xl font-extrabold mb-2">SAP Production Planning</h1>
                <div className="flex flex-wrap gap-3 text-sm">
                  <span className="bg-white/20 px-3 py-1 rounded-full">{cert.questions} questions</span>
                  <span className="bg-white/20 px-3 py-1 rounded-full">{cert.duration} min</span>
            ...
            {!isPro && isAuthenticated && (
              <div className="mt-4 bg-white/10 border border-white/20 rounded-xl p-3 flex items-center justify-between gap-3">
                ...
                <button onClick={handleUpgrade} className="flex-shrink-0 px-4 py-1.5 bg-white text-rose-700 rounded-xl text-xs font-bold hover:bg-white/90 transition-colors">
                  Activer Pro (gratuit)
                </button>
              </div>
            )}
            {!isAuthenticated && (
              <div className="mt-4 bg-white/10 border border-white/20 rounded-xl p-3 flex items-center justify-between gap-3">
                ...
                <Link to="/register" className="flex-shrink-0 px-4 py-1.5 bg-white text-rose-700 rounded-xl text-xs font-bold hover:bg-white/90 transition-colors">
              ...
              <aside className="hidden lg:block w-72 flex-shrink-0 sticky top-[5rem]">
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 flex items-start gap-3 transition-colors
                            ${isActive ? "bg-rose-600/10 dark:bg-rose-600/20" : "hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                        >
                          <div className={`h-6 w-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold
                            ${isActive ? "bg-rose-600 text-white" : accessible ? "bg-gray-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300" : "bg-gray-100 dark:bg-slate-700 text-slate-400"}`}>
                            {accessible ? idx + 1 : <LockIcon />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={`text-xs font-semibold leading-tight ${isActive ? "text-rose-600 dark:text-rose-400" : accessible ? "text-slate-800 dark:text-slate-200" : "text-slate-400 dark:text-slate-500"}`}>
                              {ch.title}
                            </p>
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {ch.isPremium && !accessible && (
                                <span className="text-xs bg-rose-600/10 text-rose-600 dark:text-rose-400 px-1.5 py-0.5 rounded font-semibold">Pro</span>
                              )}
                            </div>
                          </div>
                        </button>
                  ...
                  <div className="px-4 py-3 border-t border-gray-100 dark:border-slate-700">
                    {canAccess(true) ? (
                      <Link to="/certifications/pp/examen"
                        className="block w-full text-center py-2.5 bg-rose-600 text-white rounded-xl text-sm font-semibold hover:bg-rose-700 transition-colors">
              ...
              <main className="flex-1 min-w-0">
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                    ...
                    <div className="px-6 pt-4 flex flex-wrap gap-2 border-b border-gray-100 dark:border-slate-700 pb-3">
                      {chapter.lessons.map((l) => (
                        ...
                        >
                          {completedLessons.has(l.id) && <span className="text-rose-400"><CheckIcon /></span>}
                ...
                <div className="lg:hidden mt-4 bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 border-b border-gray-50 dark:border-slate-700/50 last:border-0 flex items-center gap-3 transition-colors
                          ${ch.id === activeChapter ? "bg-rose-600/10" : "hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                      >
                        <span className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0
                          ${ch.id === activeChapter ? "bg-rose-600 text-white" : "bg-gray-100 dark:bg-slate-700 text-slate-500"}`}>
                          {accessible ? idx + 1 : <LockIcon />}
                        </span>
                        <span className={`text-sm font-medium ${ch.id === activeChapter ? "text-rose-600 dark:text-rose-400" : accessible ? "text-slate-800 dark:text-slate-200" : "text-slate-400"}`}>
                          {ch.title}
                        </span>
                        {ch.isPremium && !accessible && (
                          <span className="ml-auto text-xs bg-rose-600/10 text-rose-600 px-2 py-0.5 rounded font-semibold">Pro</span>
                        )}
                      </button>
                    );
                  })}
                  <div className="px-4 py-3">
                    {canAccess(true) ? (
                      <Link to="/certifications/pp/examen" className="block w-full text-center py-2.5 bg-rose-600 text-white rounded-xl text-sm font-semibold">
                        🎯 Simuler l'examen
                      </Link>
                    ) : (
