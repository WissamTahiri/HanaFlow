...
import { useGamification, BADGES, LEVELS, getLevelInfo } from "../context/GamificationContext.jsx";
  ...
  return (
    <>
      <SEO
        ...
        description="Vos badges et votre progression sur HanaFlow. Débloquez des achievements en étudiant les modules SAP et en passant les certifications."
        path="/achievements"
      />
      <div className="min-h-[calc(100vh-4rem)] bg-gray-50 dark:bg-sapDark">

        {/* Hero */}
        <div className="bg-gradient-to-br from-slate-900 via-amber-900 to-orange-700 text-white">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {catBadges.map((badge) => {
                    ...
                    return (
                      <div
                        key={badge.id}
                        className={`bg-white dark:bg-slate-800 rounded-2xl border p-4 text-center transition-all
                          ${unlocked
                            ? "border-amber-200 dark:border-amber-800/50 shadow-sm"
                            : "border-gray-200 dark:border-slate-700 opacity-50 grayscale"
                          }`}
                      >
                        ...
                        {unlocked && (
                          <p className="text-xs text-emerald-500 font-semibold mt-1.5">✓ Débloqué</p>
                        )}
                      </div>
                    );
                  })}
